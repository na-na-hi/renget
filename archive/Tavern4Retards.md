...
:-: | :-
**Swipes** | Under the “Avatar” tab, you can turn on the “Swipe” function. It lets you regenerate the latest message for a different one. Just click the arrow on the right side of the message. You can "Swipe" back to the previous messages on the left side. But be careful, you can only swipe the latest message. If you send a new message, you will lose the previous swipes. This feature is good for Turbo, as it often will miss the nuances of users request.
...
### Proxy link sharing!
Proxy sharing is a new thing that's being done to share keys for others. Made and published by [Idosal](https://huggingface.co/spaces/idosal/oai-proxy)(?), this allows for anons who have been gifted with many GPT keys, to share it with others to enjoy. By placing the key behind the proxy, one can control usage of keys and avoid someone getting a hold of it to manipulate through the AI. When connecting to a proxy, you are exposing your computer, network to someone's server! Which then redirects your connetion and request to OpenAI. This poses a security risk! So use them at your own risk. However, the source code is always available on Huggingface, so you can confirm if it's been alter with in a malicious way if you're tech-savy.. Which let's be honest, you're reading this guide, so you're not.

So to quickly explain how the links works, I'll provide a small summary:
>Whocars makes a huggingface account : https://huggingface.co/whocars123
>He imports Idosal's code into his own 'space' on huggingface and gives it a name.
>https://huggingface.co/spaces/whocars123/oai-proxy
>On that site we get to see the [services status (Running, Error, Pause) and information about the proxy](https://i.imgur.com/CGQlYuk.png).
>We can see the status of keys, Turbo, GPT4, Logged etc.. You want to look for this:
>"api": "https://whocars123-oai-proxy.hf.space/proxy/openai/v1",
>Strip it to just the link
>https://whocars123-oai-proxy.hf.space/proxy/openai/v1  : And now you have the actual proxy link.
>Past into tavern proxy and then add something to API key and connect.
>Ready to go.

Now, most proxies have a tendency to only show 95% of the link, like this: ``https://whocars123-oai-proxy.hf.space/proxy/openai``.
When that is the case, you need to add ``/v1`` to the link, like this: ``https://whocars123-oai-proxy.hf.space/proxy/openai/v1``.
Then it's fully functioning and you can put it into Tavern and RP with your waifu.

Now that we're all enjoyers of keys sharing. I'll share some of the proxies that are circulating around. Note I will **NOT** be keeping this up-to-date. But I will occasionally update it, as /aicg/ just burns through threads every day. 
...
I have created a CSS and Tampermonkey script for https://alwaysfindtheway.github.io/ that can help to clean up the interface, highlight the GPT4 keys, and refresh the page every 10 minutes. The script is available at https://rentry.org/4RetardsUtility. Note, the site has been changed for the worse. My script dosen't have the same effect on it anymore, but it still works for lessen some of the information.
