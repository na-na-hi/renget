...
PASSIVE
- **~~After using support skill, you can trigger your own QTE.~~** Combo ATK, Basic ATK and QTE has lower meter recovery.
- **When charging meter, gain Iron Body**
- **On entry, gain full meter.**
- When you hit the enemy with Hunting Time (the chainsaw slash spam), on fourth hit of the attack, gain a Pledge stack (CD:0.1s), Teri takes 2% less Total DMG, max 20 stacks, lasts for 5s, can be refreshed.
- **Basic ATK SEQ 4 gathers enemies.**
- **Basic ATK seq 4 and Combo ATK has improved target range. Every 20 times you deal Bleed DMG, launch a lightning strike that deals 10% ATK of Lightning DMG, counts as Melee DMG.**
...
SIGNET 3
**Each slash of Hunting Time reduces Ult CD by 0.25s.** The max limit of Pledge increased to 30 stacks. After Ultimate ends, immediately gain 30 Pledge stacks.

SIGNET 4
Meter gained from charging increased to 40, Hunting Time (the chainsaw slash spam) consume less meter, and atk speed is increased. **For each Pledge stack, gain Lightning DMG +5%.** When Pledge stack is 20 or more, Hunting Time Lightning DMG +10%. Ult deal additional dmg = Pledge stack x 10% ATK of Lightning DMG. Clears all Pledge stack when Ult ends.

SIGNET 5
Every time you enter Violent state, ~~Ult cd -7s, and~~ Valk Total DMG ~~+50%~~ **+60%** for 20s, can be refreshed.
