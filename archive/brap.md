...
**IMPORTANT:** I've recently discovered a very useful LoRA that you might want to install. It's named [Add Detail/Detail Tweaker](https://civitai.com/models/58390) and it'll improve the quality of all your generations. I'm currently testing with it, but it seems to fix a common issue when you upscale by 2 is that sometimes the image is slightly blurry. It looks like it's pretty good at fixing a lot of generation errors too, so I definitely recommend using it! Best weight I've found thus far is 0.7, but that may need more testing.
...
#### Version 2:
The art I am generating is going towards the creation of version 2 of this LoRA. It will aim to fix a lot of the issues detailed above and should be way more consistent with how the gas effects are generated. I do not know when it will be completed, but it will likely be a couple months away still. I need to look into how to fix compatibility issues with other LoRAs and checkpoints. If anyone knows the optimal settings for training a LoRA that reduces it fucking with the image when you combine it with certain checkpoints or LoRAs let me know.

#### Support me:
You can check out my socials below, thank you for reading!
