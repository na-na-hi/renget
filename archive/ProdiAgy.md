...
!!! info Folder name captions are now working. Prodigy has been calibrated to iA3. Majority of settings corrected.
!!! warning Now figuring out step scaling. iA3 should train 10x faster than a LoRa/variant. Will update once this is figured.
...
[iA3 is like TI for the UNET and done at a very small size, about 200kb, works amazing on both characters and style. Very lightweight. Consumes the least amount of VRAM. Eats a lot of learning rate for breakfast. ```iA3 can simply be thought as LoRa*10, it can be done anywhere from 30-160 steps depending on settings and dataset.``` ```Intended to improve over LoRa and its variants which it succeeds in doing.```](https://huggingface.co/docs/peft/conceptual_guides/ia3)
...
### -> ==*TL;DR: train_on_input true/false -\> d_coef if not in good d\*lr range (will immediately overtrain if wrong) -\> weight_decay 0.7-1.0 based on how well you trained*== <-
###Instructions:
...
!!! danger ==OPTIONAL:== ```Default d_coef is 1.0 (0.1-2.0), it scales the d*lr (iA3 recommended initial LR anywhere inbetween 0.004-0.009) for Prodigy.``` You can adjust this to correct your initial LR for iA3.
!!! danger  ==OPTIONAL:== ```weight_decay is very recommended for styles and small datasets.``` Keep as I set it or lower to a minimum of 0.7.
...
```
{
  ...
  "additional_parameters": "--lr_scheduler_type \"CosineAnnealingLR\" --lr_scheduler_args \"T_max=160\" \"eta_min=0.000\"",
  "cache_latents": true,
  ...
  "min_snr_gamma": 1,
  "optimizer": "Prodigy",
  "optimizer_args": "\"eps=1e-7\" \"betas=0.9,0.999\" \"d0=1e-5\" \"d_coef=1.0\" \"weight_decay=1.000\" \"safeguard_warmup=False\" \"use_bias_correction=False\"",
