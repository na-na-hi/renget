...
### -> ==*TL;DR: adjust clip_skip, set t_0 to your desired steps (40-200) wait for it to be reached. Make X/Y/Z. Keep seed unset and retry for variations.*== <-
###Instructions:
...
!!! danger  ==OPTIONAL. CLIP_SKIP IS MORE IMPORTANT:== ```Enable bias_correction to prevent overtraining (not recommended just adjust clip_skip) and/or adjust weight_decay (not recommended since bias_correction is enough), any value from 0 to 5 should provide epic results (depends on other settings and d*lr). Only becomes a requirement when your dataset artstyle is abstract (monochrome, pixel art, minimalistic, etc)```
...
```
{
  ...
  "additional_parameters": "--lr_scheduler_type \"CosineAnnealingWarmRestarts\" --lr_scheduler_args \"T_0=110\" \"T_mult=1\" \"eta_min=5e-1\"",
  "cache_latents": true,
  ...
  "min_snr_gamma": 1,
  "optimizer": "Prodigy",
  "optimizer_args": "\"growth_rate=1.1\" \"d0=5e-3\" \"d_coef=0.2\" \"weight_decay=1.0\" \"use_bias_correction=True\"",
