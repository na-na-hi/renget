...
### -> ==*TL;DR: d_coef 5-12 & train_on_input true/false -\> weight_decay 0.0-0.3 -\> OPTIONAL: min_snr_gamma 0 or 1-10 -\> OPTIONAL: clip_skip 1-3*== <-
###Instructions:
...
!!! danger ```Default d_coef is 1.0, it scales the LR for Prodigy.``` You can slightly adjust this between 5.0 - 12.0 to correct your initial LR for iA3. Most cases should work fine on 10.0 as I set it.
...
!!! note Odd datasets (monochrome, pixel, etc.) require higher weight_decay in general. ```0.6 - 1.0 can be good for said minimalistic artstyles/simple characters and even some very complex characters that may overpower things or get the model confused; 0.0 - 0.3 is good for a vast majority of things in every other situation but you can also easily go higher, you'll just have to trigger certain aspects individually if you do and your LR isn't high enough to compensate.``` (ex: on 0.8 w_d without a high enough d\*lr a robot might be adapted to a human by default on anime models and such and to trigger their robot form you'd have to prompt for the folder name trigger along with the robot token included somewhere in the prompt)
!!! danger Adjust ```min_snr_gamma if you don't get your desired result.``` Recommend leaving on 0 first (disabled/20+) then adjusting between 1 - 10: Lower learns faster and favors texture without structure, it also can lower or increase d\*lr. Higher learns slower and includes structure, it also can lower or increase d\*lr. Compensate.
