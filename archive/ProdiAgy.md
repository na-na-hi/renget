...
[```iA3 can simply be thought of as a self multiplying LoRa, it is done anywhere from 20-100 steps and provides the same if not better results than a LoRa.``` ```Intended to improve over LoRa and its variants which it succeeds in doing.```](https://huggingface.co/docs/peft/conceptual_guides/ia3)
...
### -> ==*TL;DR: adjust clip_skip, set t_0 to your desired steps (20-160) and wait for it to be reached then manually close. Make X/Y/Z. Keep seed unset and retry for variations.*== <-
###Instructions:
...
```
{
  ...
  "additional_parameters": "--lr_scheduler_type \"CosineAnnealingWarmRestarts\" --lr_scheduler_args \"T_0=160\" \"T_mult=1\" \"eta_min=5e-1\"",
  "cache_latents": true,
  ...
  "min_snr_gamma": 1,
  "optimizer": "Prodigy",
  "optimizer_args": "\"growth_rate=1.002\" \"d0=1e-2\" \"d_coef=1.0\" \"weight_decay=1\" \"use_bias_correction=True\"",
