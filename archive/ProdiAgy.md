...
[```iA3 can simply be thought of as LoRa*10, it is done anywhere from 20-80 steps and provides the same if not better results than a LoRa.``` ```Intended to improve over LoRa and its variants which it succeeds in doing.```](https://huggingface.co/docs/peft/conceptual_guides/ia3)
...
### -> ==*TL;DR: adjust clip_skip, set t_0 to your desired steps (40-100) and wait for it to be reached then manually close. Make X/Y/Z. Keep seed unset and retry for variations.*== <-
###Instructions:
...
!!! danger  ==OPTIONAL. CLIP_SKIP IS MORE IMPORTANT:== ```Adjust weight_decay, any value from 0 to 10 should provide epic results (depends on other settings and d*lr). Only becomes a requirement when your dataset artstyle is abstract (monochrome, pixel art, minimalistic, etc)```
!!! warning bias_correction is not recommended.
!!! danger ==OPTIONAL:== ```Default d_coef is 1.0, it scales the d*lr for Prodigy. Shouldn't need changing for iA3.```
!!! note ==OPTIONAL: Adjust train_batch_size.== ```This has been set to 10 here which should fit within 6GB VRAM and above when training at 512x512. Recommend standardizing 10 so that even people on low VRAM can get similar training results as you by following metadata.```
...
```
{
  ...
  "additional_parameters": "--lr_scheduler_type \"CosineAnnealingWarmRestarts\" --lr_scheduler_args \"T_0=50\" \"T_mult=1\" \"eta_min=5e-1\"",
  "cache_latents": true,
  ...
  "min_snr_gamma": 1,
  "optimizer": "Prodigy",
  "optimizer_args": "\"growth_rate=1.004\" \"d0=5e-3\" \"d_coef=1.0\" \"weight_decay=1.0\"",
