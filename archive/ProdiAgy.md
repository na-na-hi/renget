...
!!! danger ==IMPORTANT:== ```train_on_input means training IN blocks (good for characters), disabling it means training OUT blocks (good for styles). Recommend to train on each and X/Y/Z.```
!!! danger  ==OPTIONAL:== ```Adjust weight_decay, any value from 0 to 1 should provide epic results (depends on other settings and d*lr). Only becomes a requirement when your dataset artstyle is abstract (monochrome, pixel art, minimalistic, etc)```
...
```
{
  ...
  "additional_parameters": "--lr_scheduler_type \"CosineAnnealingWarmRestarts\" --lr_scheduler_args \"T_0=100\" \"T_mult=1\" \"eta_min=5e-1\"",
  "cache_latents": true,
  ...
  "min_snr_gamma": 1,
  "optimizer": "Prodigy",
  "optimizer_args": "\"growth_rate=1.005\" \"d0=1e-2\" \"d_coef=1.0\" \"weight_decay=1\" \"use_bias_correction=True\"",
