...
To find the opposing prompts that can keep the logit of the hidden prompt relatively the same, I came across 2 prompts like that.
```
...
The first one, the repeated qs, will ramp down most tokens in your story since the hidden prompt will predict "q" with high logit most of the time.
The second one, the random sequence of characters, will ramp up most tokens in your story since the hidden prompt couldn't predict the next token well because they are random. The sequences is chosen from a base64-encoding of AES256-encrypted text, which should be as close to randomness (and hence, maximize the randomness in the prompt) as we can.

To use/test this new alternative temperature, set temperature to 1 (to disable it). And then choice either the repeated qs or random sequences above. The repeated qs should act like high randomness while the random sequences should act like low randomness.

If you look at token probabilities, you may find that some tokens don't act like I described. That's because the CFG works on logit, not the probability that the UI shows you. Also, other sampling methods may cause unintended effects on using CFG this way.

This method of using CFG is highly experimental, if you have feedback or something, just tell me in the thread.
