#`Modèles pour Stable Diffusion`
!!! note  **Tous les remerciements vont à Plonk pour la page original : (https://rentry.org/sdmodels)**
[TOC2]

## Contact
(FR) Je me suis moins tenu au courant de SD ces derniers temps, ce qui signifie que je suis susceptible de manquer de nouveaux modèles.
Si vous pensez que quelque chose manque dans cette liste, contactez plonk, **en anglais uniquement**, avec des détails.
plonk#2327

Date de mise à jour de la traduction française : 12/12/22
Date de la dernière entrée de la page original : 26/11/22

## Liste des modifications
Les entrées les plus récentes en premier.

26/11/2022
>\+ Stable Diffusion v2.0
15/11/2022
>\+ easter_e9 [b56f765a]
14/11/2022
>\+ SmirkingFace > EBL 1.1 [967237c9]
>\+ Dreambooth > hyperpreg: v1.1 (realism)
13/11/2022
>\+ Dreambooth > Complex-Lineart
12/11/2022
>\+ Dreambooth > gigachad-diffusion
>\+ Dreambooth > dbmai [e02601f3]
>\+ Fusion > HassansBlend
>\+ Dreambooth > ykgl.ckpt (y2k cgi girls)
>\+ Pyro's POV Cowgirl Model
11/11/2022
>\+ Dreambooth > Pepestyle
>\+ Dreambooth > jaggy92500.ckpt [93423a00]
>\+ Dreambooth > CSRmodel (cutesexyrobutts) [b77538cc]
10/11/2022
>\+ Dreambooth > copeseethemaldchinai_10000.ckpt (samdoesart) [32186669] 
>\+ Cafe Unofficial Instagram TEST Model [50b987ae]
>\+ Dreambooth > Black Souls bs_1500.ckpt [37ec0dc9]
>\+ Dreambooth > CopeSeetheMald-berry200_20400.ckpt (samdoesart) [fa49a214]
>\+ Dreambooth > CopeSeetheMald-200_20400.ckpt (samdoesart) [95f071f9]
>\+ Fusion > samdoartsultmerge.ckpt [8687d7a5]
>\+ Dreambooth > irlhyperbreasts_9k.ckpt [5ce8b6ec]
09/11/2022
>\+ Dreambooth > JWST Deep Space Diffusion
>\+ Dreambooth > samdoesarts_style [85b77ff9]
>\+ Dreambooth > midjourney-v4-diffusion
>\+ Dreambooth > BloodborneDiffusion
>\+ Dreambooth > SD_PixelArt_SpriteSheet_Generator
>\+ Dreambooth > Comic Diffusion
>\+ Dreambooth > VTT RPG 
>\+ Fusion > BerryMix [19810fe6]
>\+ Fusion > FreckleGuy's mix
08/11/2022
>\+  Anything-V3.0 [38c1ebe3] [1a7df6b8] [6569e224]
>\+ SXD-v0.8
>\+ Dreambooth > Gyokai/onono imoko 
>\+ Fusion > MMDv1-18
>\+ Dreambooth > futacum_r34.ckpt
>\+ Dreambooth > futanari_v2_e3_s10000.ckpt
>\+ Dreambooth > Futa_step_3200_10_27.ckpt
06/11/2022
>\+ Dreambooth > Pixel Art V1
>\+ Dreambooth > App Icons Generator V1
>\+ Dreambooth > MicroWorlds 
>\+ Dreambooth > Pixel Landscapes V1
>\+ Dreambooth > Cal Arts Style
>\+ Dreambooth > Belle Dephine [32cbf6c8]
>\+ Dreambooth > Latex's Abandoned Style
04/11/2022
>\+ SnackBar-General-V1-E11 [16d5a5c7]
>\+ easter_e5 [da453598]
>\+ easter_e4 [3997b596]
>\+ Dreambooth > hyperass v1
03/11/2022
>\+ Bump version Dreambooth > Zeipher F222
02/11/2022
>\+ Dreambooth > oshinobu-pruned.ckpt
>\+ Dreambooth >henreader_final_pruned.ckpt [e5803978]
>\+ Dreambooth > oshino_shinobu_final_pruned.ckpt
01/11/2022
>\+ Easter e3 [9c5ade34] 
>\+ Pyro's Blowjob Model v1.0 [9b5251e8]
31/10/2022
>\+ Dreambooth > DCAU [2fd6057b]
>\+ Dreambooth > bukkake [b4a14787]
>\+ Contact Ajouté
>\+ Dreambooth > classic-anim-diffusion [be7ddafc]
>\+ Dreambooth > hyperbreasts: v3
>\+ Dreambooth > hyperpreg: v2
29/10/2022
>\+ Pachu-Diffusion [54d5d199]
>\+ Dreambooth > Arcane-Diffusion [318a302e]
>\+ Dreambooth > modern-disney-diffusion [ccf3615f]
>\+ Dreambooth > elden-ring-diffusion [16d77205]
>\+ pony-diffusion-v2 [5bdc40aa]
>\+ cookie_sd_pony_run_a12 [2ce7dcf5] [67ff5385]
26/10/2022
>\+ Dreambooth > Wayne Barlowe [e0c27550]
25/10/2022
>\+ Liens vers les instructions d'utilisation de SmirkingFace (EBL sans l'interface d'AUTO)
>\+ Gape > gape60 [25396b85]
>\+ SmirkingFace > EB 1.0 [7f115f17] [7010a578] [339b9a04]
>\+ SmirkingFace > EBL 1.0 [758fda47]
24/10/2022
>\+ Dreambooth > Reverse cowgirl s4629674.ckpt [e285da0d]
>\+ sd-wikiart-v2.ckpt. [8f32b8df]
>\+ Dreambooth > Kurisu Visual Novel Official artist (Huke) [eda12c8e]
22/10/2022
>\+ Dreambooth > dreambooth_lain_girl.ckpt [e7629bf8]
>\+ Dreambooth > LOKEAN_MISSIONARY_POV.ckpt [e02601f3]
>\+ Dreambooth > LOKEAN_PUPPYSTYLE_POV.ckpt [e02601f3]
21/10/2022
>\+ Stable Diffusion VAE ft-mse-original
>\+ Stable Diffusion VAE ft-ema-original
20/10/2022
>\+ Stable Diffusion v1.5 [81761151] [a9263745]
19/10/2022
>\+ Dreambooth > Blacked POV blowjob [8467f44f]
>\+ Stable Diffusion Inpainting v1.5 [3e16efc8]
18/10/2022
>\+ TestFurry_5.ckpt [b1f1855e]
>\+ Trinart > trinart_characters_19.2m_stable_diffusion_v1 [d64dab7f] + VAE
>\+ Dreambooth > Kurisu
16/10/2022
>\+ Waifu Diffusion VAE kl-f8-anime2
14/10/2022
>\+ Bump version Dreambooth > Zeipher F111
>\+ Hentai Diffusion > HD-16.ckpt [e2ec4647]
>\+ Dreambooth > hyper_preg_2.7k-lr5e-7-15ks-fp16.ckpt
>\+ Dreambooth > Arcane Vi vimod.ckpt [e02601f3]
12/10/2022
>\+ Anal Vore AVHumanFurryPony7.ckpt [68e2e32d]
>\+ Dreambooth > StudioGhibli [10c6aa67]
11/10/2022
>\+ Dreambooth > towa-4000-pruned-fp16.ckpt [d578d3c1]
>\+ Dreambooth > Star Fox Krystal 
>\+ Ajout des tags de r34 e4
10/10/2022
>\+ Waifu Diffusion VAE kl-f8-anime
09/10/2022
>\+ R34 > r34_e4.ckpt [6e45cf2c]
>\+ Dreambooth > mori_calliope-4000-pruned-fp16.ckpt [6cb50083]
>\+ Dreambooth > gawr_gura-4000-pruned-fp16.ckpt [cbd4da60]
>\+ R34 > r34_e3.ckpt [57ea6f43]
>\+ Dreambooth > nagatoro-4000-pruned-fp16.ckpt [8c5ff341]
08/10/2022
>\+ Ajout d'un lien vers WD V1.3 dans les notes de publication https://gist.github.com/harubaru/f727cedacae336d1f7877c4bbe2196e1
07/10/2022
>\+ R34 > r34_e2.ckpt [778b68b1]
>\+ Dreambooth > hilda-v2-8000-pruned-fp16.ckpt [d2c8eef1]
06/10/2022
>\+ Dreambooth > Zeipher Female Nude (better anatomy)
>\+ Dreambooth > Zeipher BB (Blowjob images)
>\+ Dreambooth > Emilia_CustReg_3k.ckpt [2660cf9a]
>\+ Dreambooth > Gawr_Gura_450img.ckpt [e7883abf]
>\+ Dreambooth > misato-10000-pruned-fp16.ckpt [c51e4c77]
>\+ Dreambooth > 2b-10000-pruned-fp16.ckpt [8cf4478f]
>\+ Dreambooth > bea-14000-pruned-fp16.ckpt [802a681b]
>\+ Dreambooth > rizaDB-54imgs-4500steps.ckpt
>\+ Dreambooth > nanachiDB-42imgs-5000steps.ckpt
>\+ Gape > gape22 [d69e1a23]
>\+ Gape > gape18
>\+ Trier les versions du modèle par les plus récents
>\+ Waifu Diffusion v1.3 Full [84692140] [4470c325] [e1de58a9] [3e1a125f]
05/10/2022
>\+ Waifu Diffusion v1.3 beta epoch09 [a0d90b41] [134eaffb] [e90a9d02]
04/10/2022
>\+ R34 > r34_e1.ckpt [f9000e4e]
>\+ Hentai Diffusion > RD1412.ckpt [2140af02] [5b87f0e6] [4fdde306]
>\+ Waifu Diffusion v1.3 beta epoch08 [55537280] [7dd9e8e1] [d12b4159]
03/10/2022
>\+ Hentai Diffusion > RD1212.ckpt [a1f5a67e] [37b5398c] [3b3459c8]
>\+ Bare Feet / Full Body b4_t16_noadd [03e33bb4] [2bd8166c] [9012c514]
>\+ Waifu Diffusion v1.3 beta epoch07 [3ad68e9a] [e4150f29] [39312118]

## Stable Diffusion
### Stable Diffusion v2.0
Code de Stable Diffusion 2.0
https://github.com/Stability-AI/stablediffusion

Modèle 768x768
https://huggingface.co/stabilityai/stable-diffusion-2

Modèle de base 512x512
https://huggingface.co/stabilityai/stable-diffusion-2-base

Modèle de profondeur
https://huggingface.co/stabilityai/stable-diffusion-2-depth

upscaler 4x
https://huggingface.co/stabilityai/stable-diffusion-x4-upscaler

### Stable Diffusion v1.5 [81761151] [a9263745]
>Le checkpoint de Stable-Diffusion-v1-5 a été initialisé avec les poids du point de contrôle Stable-Diffusion-v1-2 et ensuite affiné sur 595k étapes à la résolution 512x512 sur "laion-aesthetics v2 5+" et une baisse de 10% du conditionnement du texte pour améliorer l'échantillonnage de guidage sans classificateur.
Dépôt https://huggingface.co/runwayml/stable-diffusion-v1-5

!!! note
	v1-5-pruned-emaonly.ckpt **[81761151]** | **SHA256** cc6cb27103417325ff94f52b7a5d2dde45a7515b25c255d8e396c90014281516
	v1-5-pruned.ckpt **[a9263745]** | **SHA256** e1441589a6f3c5a53f5f54d0975a18a7feb7cdf0b0dee276dfc3331ae376a053

**Torrent v1-5-pruned-emaonly.ckpt**
```text
magnet:?xt=urn:btih:2daef5b5f63a16a9af9169a529b1a773fc452637&dn=v1-5-pruned-emaonly.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web v1-5-pruned-emaonly.ckpt**
Besoin de s'identifier.
```text
https://huggingface.co/runwayml/stable-diffusion-v1-5/resolve/main/v1-5-pruned-emaonly.ckpt
```

**Torrent v1-5-pruned.ckpt**
!!! warning
	Cette version du modèle n'est utile que pour l'entraînement. Si vous ne savez pas ce que c'est, vous n'en avez pas besoin.
	Si vous voulez seulement générer des images, ou entraîner Dreambooth/Inversion textuelle/hyper-réseau, v1-5-pruned-emaonly.ckpt à la place.

```text
magnet:?xt=urn:btih:38c2b7a8abf55604ae8a5f060ffda2e028075af3&dn=v1-5-pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web v1-5-pruned.ckpt**
Besoin de s'identifier.
!!! warning
	Cette version du modèle n'est utile que pour l'entraînement. Si vous ne savez pas ce que c'est, vous n'en avez pas besoin.
	Si vous voulez seulement générer des images, ou entraîner Dreambooth/Inversion textuelle/hyper-réseau, v1-5-pruned-emaonly.ckpt à la place.
```text
https://huggingface.co/runwayml/stable-diffusion-v1-5/resolve/main/v1-5-pruned.ckpt
```

### Stable Diffusion VAE ft-mse-original
Repris de ft-EMA, utilise les poids EMA et a été entraîné pour 280k étapes supplémentaires en utilisant une perte re-pondérée, avec plus d'emphase sur la reconstruction MSE (produisant des sorties un peu plus "lisses"). Pour garder la compatibilité avec les modèles existants, seule la partie décodeur a été affinée ; les points de contrôle peuvent être utilisés comme un remplacement immédiat de l'autoencodeur existant.

Dépôt & Infos (with comparisons) https://huggingface.co/stabilityai/sd-vae-ft-mse-original

!!! warning
	Un VAE n'est pas la même que les autres modèles de cette liste. Il ne s'agit pas d'un modèle complet qui peut être utilisé pour générer des images seul, mais d'un composant à utiliser en tandem avec d'autres modèles.
	Si vous ne savez pas ce que c'est ou pourquoi vous en avez besoin, ne vous en préoccupez pas.

**Web**
```text
https://huggingface.co/stabilityai/sd-vae-ft-mse-original/resolve/main/vae-ft-mse-840000-ema-pruned.ckpt
```

### Stable Diffusion VAE ft-ema-original
Repris à partir du point de contrôle original, entraîné pour 313198 étapes et utilise les poids EMA. Pour garder la compatibilité avec les modèles existants, seule la partie décodeur a été affinée ; les points de contrôle peuvent être utilisés comme un remplacement immédiat de l'auto-codeur existantDépôt & Infos (Avec des comparaisons) https://huggingface.co/stabilityai/sd-vae-ft-ema-original

!!! warning
	Un VAE n'est pas la même que les autres modèles de cette liste. Il ne s'agit pas d'un modèle complet qui peut être utilisé pour générer des images seul, mais d'un composant à utiliser en tandem avec d'autres modèles.
	Si vous ne savez pas ce que c'est ou pourquoi vous en avez besoin, ne vous en préoccupez pas.

**Web**
```text
https://huggingface.co/stabilityai/sd-vae-ft-ema-original/resolve/main/vae-ft-ema-560000-ema-pruned.ckpt
```

### Stable Diffusion Inpainting v1.5 [3e16efc8]
Dépôt https://huggingface.co/runwayml/stable-diffusion-inpainting
Destiné à être utilisé avec https://github.com/runwayml/stable-diffusion

>Stable-Diffusion-Inpainting a été initialisé avec les poids de Stable-Diffusion-v-1-2. D'abord 595k étapes d'entraînement régulier, puis 440k étapes d'entraînement d'inpainting à la résolution 512x512 sur "laion-aesthetics v2 5+" et un abandon de 10% du conditionnement du texte pour améliorer l'échantillonnage d'orientation sans classificateur. Pour l'inpainting, l'UNet a 5 canaux d'entrée supplémentaires (4 pour l'image masquée encodée et 1 pour le masque lui-même) dont les poids ont été initialisés à zéro après avoir restauré le point de contrôle de non-inpainting. Pendant l'entraînement, nous générons des masques synthétiques et dans 25% des masques tout.


!!! note
	**SHA256** c6bbc15e3224e6973459ba78de4998b80b50112b0ae5b5c67113d56b4e366b19

**Torrent**
```text
magnet:?xt=urn:btih:b523a9e71ae02e27b28007eca190f41999c2add1&dn=sd-v1-5-inpainting.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce
```

**Web**
Besoin de s'identifier
```text
https://huggingface.co/runwayml/stable-diffusion-inpainting/resolve/main/sd-v1-5-inpainting.ckpt
```

### Stable Diffusion v1.4 [4af45990] [7460a6fa] [06c50424]
!!! note
	sd-v1-4-pruned.ckpt **[4af45990]**
	sd-v1-4.ckpt **[7460a6fa]** | **SHA256** fe4efff1e174c627256e44ec2991ba279b3816e364b49f9be2abc0b3ff3f8556
	sd-v1-4-full-ema.ckpt **[06c50424]** | **SHA256** 14749efc0ae8ef0329391ad4436feb781b402f4fece4883c7ad8d10556d8a36a

**Torrent**
```text
magnet:?xt=urn:btih:3A4A612D75ED088EA542ACAC52F9F45987488D1C&tr=udp://tracker.opentrackr.org:1337
```
**HuggingFace**
Besoin de s'identifier.
```text
https://huggingface.co/CompVis/stable-diffusion-v-1-4-original
```
**GDrive**
```text
https://drive.google.com/file/d/1wHFgl0ivCmIZv88hVZXkb8oy9qCuaBGA/view
```

## Waifu Diffusion
### Waifu Diffusion VAE kl-f8-anime2
Une mise au point de la SD 1.4 VAE sur 250 000 images d'animation. Il est supposé améliorer la reconstruction des yeux et des doigts.
Info `https://twitter.com/haruu1367/status/1579286947519864833`
Dépôt `https://huggingface.co/hakurei/waifu-diffusion-v1-4`

!!! warning
	Un VAE n'est pas la même que les autres modèles de cette liste. Il ne s'agit pas d'un modèle complet qui peut être utilisé pour générer des images seul, mais d'un composant à utiliser en tandem avec d'autres modèles.
	Si vous ne savez pas ce que c'est ou pourquoi vous en avez besoin, ne vous en préoccupez pas.

**Web**
```text
https://huggingface.co/hakurei/waifu-diffusion-v1-4/resolve/main/vae/kl-f8-anime2.ckpt
```

### Waifu Diffusion VAE kl-f8-anime
Une mise au point de la SD 1.4 VAE sur 250 000 images d'animation. Il est supposé améliorer la reconstruction des yeux et des doigts.
Info `https://twitter.com/haruu1367/status/1579286947519864833`
Dépôt `https://huggingface.co/hakurei/waifu-diffusion-v1-4`

!!! warning
	Un VAE n'est pas la même que les autres modèles de cette liste. Il ne s'agit pas d'un modèle complet qui peut être utilisé pour générer des images seul, mais d'un composant à utiliser en tandem avec d'autres modèles.
	Si vous ne savez pas ce que c'est ou pourquoi vous en avez besoin, ne vous en préoccupez pas.

**Web**
```text
https://huggingface.co/hakurei/waifu-diffusion-v1-4/resolve/main/vae/kl-f8-anime.ckpt
```

### Waifu Diffusion v1.3 Full [84692140] [4470c325] [e1de58a9] [3e1a125f]
Modèle Waifu Diffusion 1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 10 époques.
Notes de mise à jour: https://gist.github.com/harubaru/f727cedacae336d1f7877c4bbe2196e1

!!! note
	wd-v1-3-float16.ckpt **[84692140]** | **SHA256** 4afab9126057859b34d13d6207d90221d0b017b7580469ea70cee37757a29edd
	wd-v1-3-float32.ckpt **[4470c325]** | **SHA256** 9dade826203c7ee369881b5dc20d34298fa644c1f137568e09fbc4b9a0d3e817
	wd-v1-3-full.ckpt **[e1de58a9]** | **SHA256** 23ba8d0411c211d3d14903d46613bc088924e1453ed1c6428ce86bde54a37d27
	wd-v1-3-full-opt.ckpt **[3e1a125f]** | **SHA256** 10912b9a6d773ea7c299c0563d10538ada04ade81837362b6c0c67be4df937c1

**wd-v1-3-float16.ckpt [84692140] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/wd-v1-3-float16.ckpt
```
**wd-v1-3-float32.ckpt [4470c325] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/wd-v1-3-float32.ckpt
```
**wd-v1-3-full.ckpt [e1de58a9] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/wd-v1-3-full.ckpt
```
**wd-v1-3-full-opt.ckpt [3e1a125f] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/wd-v1-3-full-opt.ckpt
```


### Waifu Diffusion v1.3 beta epoch09 [a0d90b41] [134eaffb] [e90a9d02]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 9 époques.

!!! note
	model-epoch08-float16.ckpt **[a0d90b41]** | **SHA256** b30bab13a2ea946ba85bc7b3a30c3fa20d013ca306193aa54b4771d147192dc1
	model-epoch08-float32.ckpt **[134eaffb]** | **SHA256** a16d2f57b229156a61ccab6b1cbc120332f3a2a320c75952241e6439f3fe79ec
	model-epoch08-full.ckpt **[e90a9d02]** | **SHA256** 7fb9c64bb460724dfecbf693fcd8e0e556c4664553a352c1256997813d585329

**model-epoch09-float16.ckpt [a0d90b41] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch09-float16.ckpt
```
**model-epoch09-float32.ckpt [134eaffb] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch09-float32.ckpt
```
**model-epoch09-full.ckpt [e90a9d02] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch09-full.ckpt
```

### Waifu Diffusion v1.3 beta epoch08 [55537280] [7dd9e8e1] [d12b4159]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 8 époques.

!!! note
	model-epoch08-float16.ckpt **[55537280]** | **SHA256** 3a3a36aac85502a06878ec6760c0fb95f71546ad734fbb2b6cfab9a50db7aa8d
	model-epoch08-float32.ckpt **[7dd9e8e1]** | **SHA256** e64cbc29edbef82142768c3a398345f602ba8b337ebd22cd83071162fe789a18
	model-epoch08-full.ckpt **[d12b4159]** | **SHA256** c2a8b09f77ce572c012c810495d8ae9f989d04ca6b3226c93be5dce2208bacdd

**model-epoch08-float16.ckpt [55537280] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch08-float16.ckpt
```
**model-epoch08-float32.ckpt [7dd9e8e1] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch08-float32.ckpt
```
**model-epoch08-full.ckpt [d12b4159] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch08-full.ckpt
```

### Waifu Diffusion v1.3 beta epoch07 [3ad68e9a] [e4150f29] [39312118]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 7 époques.
>Le modèle n'est pas terminé, loin de la. Ne prenez pas les résultats comme définitifs.

!!! note
	model-epoch07-float16.ckpt **[3ad68e9a]** | **SHA256** 0aa0a2d1620b65fbcfe800ac5d5b5fc681c9e3bc7373ad5519837f94828eb83f
	model-epoch07-float32.ckpt **[e4150f29]** | **SHA256** 99c1061162c126c59d17cfddaf8b475935a9f0a504f7b305b1d733e7618be82b
	model-epoch07-full.ckpt **[39312118]** | **SHA256** 74ae340acb9ac251b8b9ee57f795e18bc4d39c016272fafd440e637fe391a790

**model-epoch07-float16.ckpt [3ad68e9a] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch07-float16.ckpt
```
**model-epoch07-float32.ckpt [e4150f29] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch07-float32.ckpt
```
**model-epoch07-full.ckpt [39312118] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch07-full.ckpt
```

### Waifu Diffusion v1.3 beta epoch06 [281c28bb] [3a672a8a] [d970eaf1]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 6 époques.
>Le modèle n'est pas terminé, loin de la. Ne prenez pas les résultats comme définitifs.

!!! note
	model-epoch06-float16.ckpt **[281c28bb]** | **SHA256** c0397df53cd4cf1cfadb95a121a67eaad903e4203d708abfe3409a832a517191
	model-epoch06-float32.ckpt **[3a672a8a]** | **SHA256** f34c89c80e91d77c06df178d292611ac7757728dd7a3aaff8c4c9117e6e92e35
	model-epoch06-full.ckpt **[d970eaf1]** | **SHA256** cc5569691c843f64cc8e819b78c0081f6e8024432f6de898f11ee816ac3ff5ca

**model-epoch06-float16.ckpt [281c28bb] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch06-float16.ckpt
```
**model-epoch06-float32.ckpt [3a672a8a] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch06-float32.ckpt
```
**model-epoch06-full.ckpt [d970eaf1] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch06-full.ckpt
```

### Waifu Diffusion v1.3 beta epoch05 [25f7a927] [3563d59f] [9453d3e5]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 5 époques.
>Le modèle n'est pas terminé, loin de la. Ne prenez pas les résultats comme définitifs.

!!! note
	model-epoch05-float16.ckpt **[25f7a927]** | **SHA256** 26cf2a2e30095926bb9fd9de0c83f47adc0b442dbfdc3d667d43778e8b70bece
	model-epoch05-float32.ckpt **[3563d59f]** | **SHA256** 694617b2145abcd248d78ea2c556af86787ef9f79e9d9a0f7473d7b408dd3dfa
	model-epoch05-full.ckpt **[9453d3e5]** | **SHA256** e0fceaac42328912ed60957c5fed129a418bdd5f3ebe82d5f09e7901a296a6ce

**model-epoch05-float16.ckpt [25f7a927] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch05-float16.ckpt
```
**model-epoch05-float32.ckpt [3563d59f] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch05-float32.ckpt
```
**model-epoch05-full.ckpt [9453d3e5] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch05-full.ckpt
```

### Waifu Diffusion v1.3 beta epoch04 [958859c3] [84d80299] [994d2e0f]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 4 époques.
>Le modèle n'est pas terminé, loin de la. Ne prenez pas les résultats comme définitifs.

!!! note
	model-epoch04-float16.ckpt **[958859c3]** | **SHA256** 3fd4bbb7ca76c806d88061f0ce994b763693369bd6ea37863c93c1c6b9d25232
	model-epoch04-float32.ckpt **[84d80299]** | **SHA256** 5237503095524512c7612f81ef786036aa001fd76693064fb00478115659a335
	model-epoch04-full.ckpt **[994d2e0f]** | **SHA256** eee5a4391d6074f08bc6c3de3457c245252bde101e3e651c27b5cf85d4926b08

**model-epoch04-float16.ckpt [958859c3] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch04-float16.ckpt
```
**model-epoch04-float32.ckpt [84d80299] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch04-float32.ckpt
```
**model-epoch04-full.ckpt [994d2e0f] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch04-full.ckpt
```
### Waifu Diffusion v1.3 beta epoch03 [63e9eb31] [22ca2722] [d8b28983]
**==Bêta Inachevée==** Modèle Waifu Diffusion V1.3 entraîné sur 600 000 images haute résolution de Danbooru pour 3 époques.
>Le modèle n'est pas terminé, loin de la. Ne prenez pas les résultats comme définitifs.

!!! note
	model-epoch04-float16.ckpt **[63e9eb31]** | **SHA256** 123d8ae9cdbf08f5e68b6eaf868483c29c3408d29bce693ea12c40f9ac34b04d
	model-epoch04-float32.ckpt **[22ca2722]** | **SHA256** 1fe36906fff1a14879f1bda49d19112f0a21dd183f8f5842ea493f4733070205
	model-epoch04-full.ckpt **[d8b28983]** | **SHA256** 950ea42feb58dcdfad26f23cd5e68ea38f9df2f49449c694a389d18fb33e6ea6

  **model-epoch03-float16.ckpt [63e9eb31] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch03-float16.ckpt
```
**model-epoch03-float32.ckpt [22ca2722] Web**
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch03-float32.ckpt
```
**model-epoch03-full.ckpt [d8b28983] Web**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version float16 ou float32.
```
https://huggingface.co/hakurei/waifu-diffusion-v1-3/resolve/main/model-epoch03-full.ckpt
```


### Waifu Diffusion v1.2 [0b8c694b] [45dee52b]
Entraîné sur Stable Diffusion v1.4 en utilisant 56k images de Danbooru pour 5 époques..
!!! note
	wd-v1-2-full-ema-pruned.ckpt **[0b8c694b]** | **SHA256** 5512bc271c31fd9433494f3fced009d3b27515cca5ef1a1ed13bce136f8e8cad
	wd-v1-2-full-ema.ckpt **[45dee52b]** | **SHA256** 89138c3a7783a2b17730090c06e2e1950fb421068cc5f4a5d6c4ba1040a7e1bb
	
**Pruned Torrent [0b8c694b]**
```text
magnet:?xt=urn:btih:153590fd7e93ee11d8db951451056c362e3a9150&dn=wd-v1-2-full-ema-pruned.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
**Full EMA Torrent [45dee52b]**
La taille du fichier de l'EMA complète est plus importante car elle contient des données supplémentaires utilisées pour **l'entraînement uniquement.** 
Si vous ne savez pas de quoi il s'agit, utilisez la version taillée.
```text
magnet:?xt=urn:btih:f45cecf4e9de86da83a78dd2cccd7f27d5557a52&dn=wd-v1-2-full-ema.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

## Anything-V3.0 [38c1ebe3] [1a7df6b8] [6569e224]
Modèle pour Anime
Infos https://www.bilibili.com/read/cv19603218

**Torrent fp16 [38c1ebe3]**
```text
magnet:?xt=urn:btih:45cd353ac4fa87098db5e3a6a349539710a3a1f5&dn=Anything-V3.0-fp16.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce
```

**Torrent fp32 [1a7df6b8]**
```text
magnet:?xt=urn:btih:d9db662ab5ace77004b3348c23c9381380c27156&dn=Anything-V3.0-fp32.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce
```

**Torrent Full EMA [6569e224]**
```text
magnet:?xt=urn:btih:80460036625fb61dce4bc6e7dab744744309a2a0&dn=Anything-V3.0-fullema.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce
```

## Cafe Unofficial Instagram TEST Model [50b987ae]
>Entraîné sur ~140k images Instagram composées principalement de comptes japonais (mélange de cosplay, modèle, et comptes personnels). 
>Bien que le modèle puisse créer des images Instagram-esques réalistes à lui seul, il est recommandé de le fusionner avec un autre modèle (tel que berry ou autre) pour en tirer tout le potentiel. 

**Torrent**
```text
magnet:?xt=urn:btih:eb085b3e22310a338e6ea00172cb887c10c54cbc&dn=cafe-instagram-unofficial-test-epoch-9-140k-images-fp32.ckpt&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Fopentor.org%3A2710&tr=udp%3A%2F%2Ftracker.ccc.de%3A80&tr=udp%3A%2F%2Ftracker.blackunicorn.xyz%3A6969&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969
```

## Trinart
### trinart_characters_19.2m_stable_diffusion_v1 [d64dab7f] + VAE
>Un modèle de base de Stable Diffusion v1, entraîné par environ 19,2 millions d'images de style anime/manga (y compris les images augmentées préenregistrées) et affiné avec environ 50 000 images. Ce modèle cherche à trouver le juste milieu entre la polyvalence du style artistique et la qualité anatomique spécifique du modèle SDv1.
>Il s'agit du même modèle version 1 qui a été publié dans le service AI Novelist/TrinArt de début septembre à octobre 14.
>L'ensemble de données est filtré pour exclure les contenus dangereux ou NSFW.


Dépôt https://huggingface.co/naclbit/trinart_characters_19.2m_stable_diffusion_v1

!!! note
	trinart_characters_it4_v1.ckpt [d64dab7f] **SHA256** d58058f2c71612aa7492d63ad8e6c32b3159494dc51a29ffe71937cdc49b6681
	autoencoder_kl-f8-trinart_characters.ckpt **SHA256** d2dd1c82220e31a72bd9958dda249ed7f94faf875d5123ae3aab7a1950a82a8f

**Web**
```text
https://huggingface.co/naclbit/trinart_characters_19.2m_stable_diffusion_v1/resolve/main/trinart_characters_it4_v1.ckpt
```

**VAE Web**
```text
https://huggingface.co/naclbit/trinart_characters_19.2m_stable_diffusion_v1/resolve/main/autoencoder_kl-f8-trinart_characters.ckpt
```

### trinart2_step115000.ckpt [f1c7e952]
Le point de contrôle V2 utilise "dropouts", 10 000 images de plus et une nouvelle stratégie d'étiquetage. Il a été entraîné plus longtemps pour améliorer les résultats tout en conservant l'esthétique originale.
!!! note
	**SHA256** 776af18775dfccf29725a994df855e7d8f7b8ea525013e3a466f210ec15c8fd4

**Web**
```text
https://huggingface.co/naclbit/trinart_stable_diffusion_v2/resolve/main/trinart2_step115000.ckpt
```

### trinart2_step95000.ckpt [cf0bd941]
Le point de contrôle V2 utilise "dropouts", 10 000 images de plus et une nouvelle stratégie d'étiquetage. Il a été entraîné plus longtemps pour améliorer les résultats tout en conservant l'esthétique originale.
!!! note
	**SHA256** c1799d22a355ba25c9ceeb6e3c91fc61788c8e274b73508ae8a15877c5dbcf63

**Web**
```text
https://huggingface.co/naclbit/trinart_stable_diffusion_v2/resolve/main/trinart2_step95000.ckpt
```

### trinart2_step60000.ckpt [6ecd8e48]
Le point de contrôle V2 utilise "dropouts", 10 000 images de plus et une nouvelle stratégie d'étiquetage. Il a été entraîné plus longtemps pour améliorer les résultats tout en conservant l'esthétique originale.
!!! note
	**SHA256** 0acc70d6a515cdedbc840e2c06525709a83cb9be8cdfdf1136a6548e9ee8f0fa

**Web**
```text
https://huggingface.co/naclbit/trinart_stable_diffusion_v2/resolve/main/trinart2_step60000.ckpt
```

### trinart_stable_diffusion_epoch3.ckpt [9d7f05fc]
Considéré comme obsolète, se référer à trinart2.
trinart_stable_diffusion est un modèle de SD affiné par environ 30 000 images assorties de haute résolution de style manga/anime pendant 3,5 époques.
!!! note
	**SHA256** 840bbc99a603d4171963b31eec700abcb6f9545d1374b53eb7f830261494be19

**Web**
```text
https://huggingface.co/naclbit/trinart_stable_diffusion/resolve/main/trinart_stable_diffusion_epoch3.ckpt
```

## SmirkingFace
>SmirkingFace est un projet qui vise à affiner Stable diffusion avec du contenu érotique de haute qualité et à lui permettre de générer une grande variété de contenu érotique. 
>Au cas où vous ne seriez pas sûr du modèle dont vous avez besoin, le modèle EB 1.0 EMA a tendance à donner de bons résultats dans la majorité des cas. Le modèle EBL 1.0 a été entraîné le plus longtemps, mais il faudra du code pour que notre modèle modifié fonctionne (publié sur notre github).
>Note : Tous nos modèles incluent la mise à jour du MSE VAE publié par Stability AI.

Info https://www.patreon.com/posts/released-models-73689798
Instructions d'utilisation avec aperçus https://www.patreon.com/posts/73775030
Liste de mot https://www.patreon.com/posts/word-lists-73913464

### EB 1.0 [7f115f17] [7010a578] [339b9a04]
Modèle utilisant l'architecture originale de diffusion stable entraînée sur une grande variété de photos érotiques avec une grande variété de poses.
Info https://www.patreon.com/posts/released-models-73689798
Instructions d'utilisation avec aperçus https://www.patreon.com/posts/73775030

##### SF_EB_1.0_ema_vae.ckpt [7f115f17]
!!! note
	SF_EB_1.0_ema_vae.ckpt [7f115f17] **SHA256** 245cac02fa7243a83ff5f7cf78407a505308e8b6f2dacdff2ad43a1846514f3d

**Torrent**
```text
magnet:?xt=urn:btih:a0bcc397b2c3c101755245fd9b26ae1aa8d5d580&dn=SF_EB_1.0_ema_vae.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/32F7ecE0y5/SF_EB_1.0_ema_vae_zip
```

**Web**
```text
https://gofile.io/d/MYOKl8
```

##### SF_EB_1.0_noema_vae.ckpt [7010a578]
!!! note
	SF_EB_1.0_noema_vae.ckpt [7010a578] **SHA256** 344719ce863db1464ee81f2dc61375c10fcb9fd8b72afd69ad07b091fc834170

**Torrent**
```text
magnet:?xt=urn:btih:3d73f553ce18ec0bae3811edb37a904c5e82c56d&dn=SF_EB_1.0_noema_vae.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/t1Iee0E4y0/SF_EB_1.0_noema_vae_zip
```

**Web**
```text
https://gofile.io/d/MYOKl8
```

##### SF_EB_1.0_pruned_vae.ckpt [339b9a04]
!!! note
	SF_EB_1.0_pruned_vae.ckpt [339b9a04] **SHA256** c50ab94df348e43ad001bd18bcbf4c0f239afc408b134d24ce74b5b7c86b3e0d

**Torrent**
```text
magnet:?xt=urn:btih:875cb7ced0127c67f466a08de8945b5f936fc020&dn=SF_EB_1.0_pruned_vae.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/icT4e1Eeyf/SF_EB_1.0_pruned_vae_zip
```

**Web**
```text
https://gofile.io/d/MYOKl8
```

### EBL 1.1 [967237c9]
Modèle utilisant une architecture étendue (1.8 milliards de paramètres), reprise de EB 1.0. Entraîné sur une grande variété de photos érotiques avec une grande variété de poses.
Ne fonctionne pas directement avec l'interface web (actuellement, du moins). 
Code https://github.com/smirkingface/stable-diffusion
Infos & aperçu https://www.patreon.com/posts/ebl-1-1-released-74653550
Faire tourner avec l'interface web https://www.patreon.com/posts/running-extended-73768212

!!! note
	SF_EBL_1.1_vae.ckpt **SHA256** 1276a1920b29b773b8c61944c9bbcce7de47e71f097633d00ba27a7013477336  

**Torrent**
```text
magnet:?xt=urn:btih:d145e9c1de7e43972ac15e360baa838ed3921bfa&dn=SF_EBL_1.1_vae.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.tracker.cl%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.jiesen.life%3a8443%2fannounce&tr=https%3a%2f%2ftracker.cyber-hub.net%3a443%2fannounce&tr=https%3a%2f%2ftr.burnabyhighstar.com%3a443%2fannounce&tr=http%3a%2f%2ftracker.mywaifu.best%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/c0bat6Hay5/SF_EBL_1.1_vae_zip
```

**Web**
```text
https://gofile.io/d/MYOKl8
```

### EBL 1.0  [758fda47]
Modèle utilisant une architecture étendue (1.8 milliards de paramètres), reprise de EB 1.0. Entraîné sur une grande variété de photos érotiques avec une grande variété de poses.
Ne fonctionne pas directement avec l'interface web (actuellement, du moins). 
Code https://github.com/smirkingface/stable-diffusion
Info https://www.patreon.com/posts/released-models-73689798
Faire tourner avec l'interface web https://www.patreon.com/posts/running-extended-73768212
Instructions d'utilisation avec aperçus https://www.patreon.com/posts/73775030

!!! note
	SF_EBL_1.0_vae.ckpt [758fda47] **SHA256** e237715adaf1b3e43eb2eee792fe82a90d01a45beb79d38807f5ca92b7a277db

**Torrent**
```text
magnet:?xt=urn:btih:8c4cafe3b148c4337e381dbc0a18fc03a2eb7ac2&dn=SF_EBL_1.0_vae.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/I6y3feEay4/SF_EBL_1.0_vae_zip
```

**Web**
```text
https://gofile.io/d/MYOKl8
```


## gg1342_testrun1_pruned.ckpt [43076286] [2ccc3e58]
Un modèle de test qui a été entraîné sur 280 images NSFW (femmes nues en solo) + 80 autres images SFW de quelques personnages fictifs.
!!! note
	gg1342_testrun1_pruned.ckpt **[43076286]** | **SHA256** ebe2f8dfeed0b87080a37e94bf0aca8800fc10691826a5e76259faf509106246
	gg1342_testrun1_pruned-float16.ckpt **[2ccc3e58]**

**Torrent**
```text
magnet:?xt=urn:btih:c95e266e15e13cf0e2d69b29338a89a94d736546&dn=gg1342_testrun1_pruned.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

## SXD-v0.8
>Modèle d'acte sexuel hardcore avec porno tentaculaire réaliste comme projet secondaire. Réalisé avec le dépôt SD de smirkingface. Les deux versions, taillée (pruned) et complète (full), sont disponibles.

**Web**
```text
https://pixeldrain.com/l/hNc8DomC
```

## Pyro's

### Pyro's Blowjob Model v1.0 [9b5251e8]
Photorealistic
>Les modèles de "blowjob" actuels basés sur les dreambooths sont nuls (heh), j'ai donc décidé de faire un " vrai " modèle et de le peaufiner de manière native (comme par exemple WaifuDiffusion l'a fait avec son modèle).
>Base model: SD 1.5
>2899 Images en entrée
>23192 étapes x taille du lot 4
>Vitesse d'apprentissage 5e-06
Info et potentiellement des versions plus récentes: https://rentry.org/pyros-sd-model

**Torrent**
```text
magnet:?xt=urn:btih:5c5d96dfb4eda3d4eb0f8f67c695b8b8ac241060&dn=pyros-bj-v1-0.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.srv00.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://anonfiles.com/6123m6F6y9/pyros-bj-v1-0_ckpt
```

**Web**
```text
https://mega.nz/file/lLtjCLwb#KgXPDzbTotcb0_quzzBMm6DaDCuSFIaF8CXxw1WsEs8
```

### Pyro's POV Cowgirl Model
Apercu: https://imgur.com/a/vgO86ZC
Info et téléchargements: https://rentry.org/pyros-pov-model

## Easter
### easter_e9 [b56f765a]
Apercu https://i.imgur.com/k4HdjgM.png
>Modèle assortie Anime/Furry
>Entraîné sur 240k images de gelbooru et e621
>Requête positive suggéré : Un artist ou arstyle aide beaucoup
>Requête négative suggéré: `letterbox, censored`

**Torrent**
```text
magnet:?xt=urn:btih:cea7216792392a6480caa61fab87257fac50f6cf&dn=easter%20epoch%209%20%5bb56f765a%5d.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopen.tracker.cl%3a1337%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt2.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/ZnhjHCza#NBk6JiSja07DL0yliVwH4dVvtmtKF3pIujueg2QPbLk
```

### easter_e5 [da453598]
>Modèle obscène (Lewd) général, "Easter.ckpt" ! Entraîné sur environ 180k images de gelbooru et e621.
>Le contenu NSFW et de niche est inclus. Je ne prends aucune responsabilité pour les résultats du modèle !
>Le modèle semble meilleur pour le premier plan que l'arrière plan.
>Requête négative suggéré: "Letterbox".
>Le nom vient de sa tendance à mélanger les lapins et les œufs.

**Torrent**
```text
magnet:?xt=urn:btih:da3395f01a1063d1980c3c2c610c25a7a20ebce8&dn=easter_e5%20%5bda453598%5d%20plus%20tags.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/1vQVHbBI#W0hrTSMy9CvUxzhWShmtGsUf4AV6ndHze5NPTLqqhKU
```

### easter_e4 [3997b596]
>Modèle obscène (Lewd) général, "Easter.ckpt" ! Entraîné sur environ 180k images de gelbooru et e621.
>Le contenu NSFW et de niche est inclus. Je ne prends aucune responsabilité pour les résultats du modèle !
>Le modèle semble meilleur pour le premier plan que l'arrière plan.
>Requête négative suggéré: "Letterbox".
>Le nom vient de sa tendance à mélanger les lapins et les œufs.

**Web**
```text
https://mega.nz/file/4vADAYYY#m7K2OVQrEYhmi5e3wUnkdb2xg6ewotysAt7Z0Rras6g
```

### easter_e3 [9c5ade34] 
>Modèle obscène (Lewd) général, "Easter.ckpt" ! Entraîné sur environ 180k images de gelbooru et e621.
>Le contenu NSFW et de niche est inclus. Je ne prends aucune responsabilité pour les résultats du modèle !
>Le modèle semble meilleur pour le premier plan que l'arrière plan.
>Requête négative suggéré: "Letterbox".
>Le nom vient de sa tendance à mélanger les lapins et les œufs.
>Actuellement à l'époque 3.

**Torrent**
```text
magnet:?xt=urn:btih:79d1adf55ab8336e095215f3d293fa3c29d57528&dn=easter_e3.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.srv00.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/l3RRiIzI#hHGR6rYAJ0-weE8kef7bBUq5u2WjI3oTP92A7atTINw
```

## Hentai Diffusion
Dépôt https://huggingface.co/Deltaadams/Hentai-Diffusion/tree/main
Il est censé être mis à jour chaque semaine, alors vérifiez le dépôt, il pourrait être plus à jour que cette liste.
Les torrents taillés (pruned) ci-dessous sont avec les poids EMA enlevés.

>Ce modèle est basé sur Waifu Diffusion 1.2, et a été entraîné sur 150 000 images de R34 et gelbooru.
>Un entraînement ciblé a été effectué sur des poses plus obscures, telles que l'accroupissement et le fait de se détourner de l'observateur, ainsi que sur l'amélioration des mains.
>Il est recommandé d'utiliser les balises du site dans requêtes. En ajoutant "1girl" au début de la requête, vous obtiendrez de meilleurs résultats.

### HD-16.ckpt [e2ec4647]
Dépôt https://huggingface.co/Deltaadams/Hentai-Diffusion/tree/main
Il est supposé être mis à jour chaque semaine, alors vérifiez le dépôt, il pourrait être plus à jour que cette liste.
!!! note
	**SHA256** 4c9317fe064386b0d8645375d0f8a0059455cfee0b84d2e280d5cc2fc9e88c21
**Web**
```text
https://huggingface.co/Deltaadams/Hentai-Diffusion/resolve/main/HD-16.ckpt
```

### RD1412.ckpt [2140af02] [5b87f0e6] [4fdde306]
Dépôt https://huggingface.co/Deltaadams/Hentai-Diffusion/tree/main
Il est supposé être mis à jour chaque semaine, alors vérifiez le dépôt, il pourrait être plus à jour que cette liste.

!!! note
	RD1412-pruned-fp16.ckpt **[2140af02]** | **SHA256** c02f07fc72ea7281a14c94469428a497b142f70f6b5b4ea20435fbf5ef3f1731
	RD1412-pruned-fp32.ckpt **[5b87f0e6]** | **SHA256** c318e7155f30af74785503d8ebf2856813e234032bd5c043c814e2f8baa4d67e
	RD1412.ckpt **[4fdde306]** | **SHA256** 477b860a6859838e99d81fa5d3345e717e11fe8bd0e7eb32bce7541d57807a4d

**Torrent RD1412-pruned-fp16.ckpt [a1f5a67e]**
```text
magnet:?xt=urn:btih:da8986f9059ce4f64f84e7390eb542558b2cd466&dn=RD1412-pruned-fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce
```

**Torrent RD1412-pruned-fp32.ckpt [37b5398c]**
```text
magnet:?xt=urn:btih:ab4c2d7308a3fa694f7409407399a1cc5d4c7ed9&dn=RD1412-pruned-fp32.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce
```

**Web Full EMA RD1412.ckpt [4fdde306]**
```text
https://huggingface.co/Deltaadams/Hentai-Diffusion/resolve/main/RD1412.ckpt
```

### RD1212.ckpt [a1f5a67e] [37b5398c] [3b3459c8]
Dépôt https://huggingface.co/Deltaadams/Hentai-Diffusion/tree/main
Il est supposé être mis à jour chaque semaine, alors vérifiez le dépôt, il pourrait être plus à jour que cette liste.

!!! note
	HenDiff_RD1212-pruned-fp16.ckpt **[a1f5a67e]** | **SHA256** 8e46950f3d700886ced3842e4f05c31a1ab30b856668e9bc990940e60ad21be1
	HenDiff_RD1212-pruned-fp32.ckpt **[37b5398c]** | **SHA256** c303f408d538544765b109aef0e2f165864cab720365a1d3a1d166237965095a
	RD1212.ckpt **[3b3459c8]** | **SHA256** 23c56765c35a9b205e52f64b46c6311279cc3a56e6d17eba32779ff590bdc3d7

**Torrent HenDiff_RD1212-pruned-fp16.ckpt [a1f5a67e]**
```text
magnet:?xt=urn:btih:f4e78d085169d2077a316bd9b75723812c1ab429&dn=HenDiff_RD1212-pruned-fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce
```

**Torrent HenDiff_RD1212-pruned-fp32.ckpt [37b5398c]**
```text
magnet:?xt=urn:btih:2a6b60f454dcf89b81e7db034fcb1536b774628c&dn=HenDiff_RD1212-pruned-fp32.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce
```

**Web Full EMA RD1212.ckpt [3b3459c8]**
```text
https://huggingface.co/Deltaadams/Hentai-Diffusion/resolve/main/RD1212.ckpt
```
**Torrent Full EMA RD1212.ckpt [3b3459c8]**
```text
magnet:?xt=urn:btih:D0B89A0516205157EA0CBDDBBB49BC60C611A3B7&dn=RD1212.ckpt&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

## Bare Feet / Full Body b4_t16_noadd [03e33bb4] [2bd8166c] [9012c514]
Labels de haute qualité uniquement (le modèle "taillé").
Grille d'aperçu https://i.imgur.com/2sJGz3j.jpeg

>Un checkpoint vraiment "en construction" axé sur les images de nus féminins de type Pieds nus (Bare Foot (BF)) et Corps entier (Full Body (FB)) - à utiliser de préférence si vous cherchez à obtenir des pieds ou des organes génitaux féminins photoréalistes précis, ce qui n'est pas toujours le cas avec le SD normal.
>Il s'agit d'un "travail en cours".
>Extrêmement spécialisé, il a perdu beaucoup de ses capacités d'usage général et de sa capacité à styliser.

**Torrent bf_fb_v3_t4_b16_noadd-ema-pruned-fp16.ckpt [03e33bb4]** 
```text
magnet:?xt=urn:btih:9530a8a0b43f83366216ab853b4419aa2056da58&dn=bf_fb_v3_t4_b16_noadd-ema-pruned-fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce
```

**Torrent bf_fb_v3_t4_b16_noadd-ema-pruned-fp32.ckpt [2bd8166c]**
```text
magnet:?xt=urn:btih:1f6bab17c548e35ac2a412e3e9119e5f4e00bb50&dn=bf_fb_v3_t4_b16_noadd-ema-pruned-fp32.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce
```

## Lewd Diffusion
### Lewd Diffusion 70k 2e [f4030c43]
LD-70k-2e-pruned.ckpt
70k images explicites de danbooru
2 époques
Base model Waifu Diffusion v1.2 
Données d'entraînement: `https://drive.google.com/drive/folders/1f_BYi88LLTZUzBHkUz8PDgw6l7M7swkd?usp=sharing`
Charactéristiques des données d'entraînement: `https://docs.google.com/spreadsheets/d/1BzNSXyT4fhiM64DwIJSCyAXuhRQ9fkxqcr-t1frIYkc/edit`
!!! note
	**SHA256** 54faf6de03fb9a6fe4d8af163b16133cd7cd045d45915178c602e4b51a92036c
**Torrent**
```text
magnet:?xt=urn:btih:U5RICVYDEJL6LIJJWFKQOIVO5GMGCJNW&dn=last-pruned.ckpt&xl=3852165809&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce
```

### Lewd Diffusion 70k 1e [950d323b]
LD-70k-1e-pruned.ckpt
70k explicit from danbooru
1 époque
Base model Waifu Diffusion v1.2 
Données d'entraînement: `https://drive.google.com/drive/folders/1f_BYi88LLTZUzBHkUz8PDgw6l7M7swkd?usp=sharing`
Charactéristiques des données d'entraînement: `https://docs.google.com/spreadsheets/d/1BzNSXyT4fhiM64DwIJSCyAXuhRQ9fkxqcr-t1frIYkc/edit`

**Torrent**
```text
magnet:?xt=urn:btih:fca8782a5a9861a6beb1aa3b48938bd1da1a665e&dn=LD-70k-1e-pruned.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
### Lewd Diffusion v0 [07734b46]
Lewd-diffusion-pruned.ckpt
Danbooru21 NSFW epoch0
40K Images
1 époque complète
Base model Waifu Diffusion v1.2
Données d'entraînement: `https://drive.google.com/drive/folders/1f_BYi88LLTZUzBHkUz8PDgw6l7M7swkd?usp=sharing`

**Torrent**
```text
magnet:?xt=urn:btih:f6976fbe3b9f93469bb62eb0c4950643b09f1f83&dn=Lewd-diffusion-pruned.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

## Yiffy
### yiffy-e18.ckpt [50ad914b]
Basé sur yiffy-e15, entraîné pour 3 époques supplémentaires en utilisant 210k images de e621 comme données d'entraînement.
>Il s'agit probablement de la dernière époque pour un certain temps, car jusqu'à ce que d'autres améliorations soient apportées à d'autres parties du réseau (telles qu'un VAE personnalisé, une résolution variable, etc.), nous pensons qu'un entraînement supplémentaire n'est pas nécessaire car le modèle a convergé vers un style décent pour le moment.
>**NOTE IMPORTANTE : ** Pendant l'entraînement, explicit a été mal orthographié en **explict**. L'utilisation de la version mal orthographiée donne de meilleures bites. Désolé :<

Tags `https://mega.nz/file/ZkkTEYJY#X_j191NtHJRp0BKJusTqmWrs1-AdAILy99mqSAMpWbo` or `https://pixeldrain.com/u/TkwJU3WG`

!!! note
	**SHA256** 64e242ae67cb1fc85110a68242c936fa6f3e33077f2bfed48c4769959dff6c61
	**MD5** dbe25794e24af183565dc45e9ec99713

**Torrent**
```text
magnet:?xt=urn:btih:b177dd04ae7062b541c82ad26f897e0a9fa514f4&dn=yiffy-e18.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```
**Web**
```text
https://sexy.canine.wf/file/yiffy-ckpt/yiffy-e18.ckpt
```
### yiffy-e15.ckpt [4bb305c0]
Basé sur yiffy-e13, entraîné pour 2 époques supplémentaires en utilisant 210k images de e621 comme donnée d'entraînement.
>**NOTE IMPORTANTE : ** Pendant l'entraînement, explicit a été mal orthographié en **explict**. L'utilisation de la version mal orthographiée donne de meilleures bites. Désolé :<
Tags `https://mega.nz/file/ZkkTEYJY#X_j191NtHJRp0BKJusTqmWrs1-AdAILy99mqSAMpWbo` or `https://pixeldrain.com/u/TkwJU3WG`

!!! note
	**SHA256** f0d49adddcb9f1030cb13e4eed21d12f27cdd3d98a9740002d0ccdb4a9fa61f1
	**MD5** cc354964edbcbb7758cb359743aa1791

**Torrent**
```text
magnet:?xt=urn:btih:2b8d5f308244eddf56d4a350df84d63045e65dd6&dn=yiffy-e15.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
**Web**
```text
https://sexy.canine.wf/file/yiffy-ckpt/yiffy-e15.ckpt
```
**Web**
```text
https://pixeldrain.com/u/qkRKKpqg
```
**Web**
```text
https://iwiftp.yerf.org/Furry/Software/Stable%20Diffusion%20Furry%20Finetune%20Models/Finetune%20models/yiffy-e15.ckpt
```
### yiffy-e13.ckpt [778b38ae]
>les 4 premières époques ont été entraînées sur ~70k images avec remplissage de lama (la cause de tous nos maux de tête, parce que le réseau a trouvé une structure pour les bordures et a commencé à le reproduire partout)
>Les 6 époques suivantes ont été entraînées sur ~120k images avec un recadrage aléatoire et un taux d'apprentissage plus faible.
>Les dernières époques ont été effectuées sur des données d'entraînement différentes, ne dépassant pas 150k images.
>**NOTE IMPORTANTE : ** Pendant l'entraînement, explicit a été mal orthographié en **explict**. L'utilisation de la version mal orthographiée donne de meilleures bites. Désolé :<
Tags `https://mega.nz/file/ZkkTEYJY#X_j191NtHJRp0BKJusTqmWrs1-AdAILy99mqSAMpWbo` or `https://pixeldrain.com/u/TkwJU3WG`

!!! note
	**MD5** bfff21ca92fb218e52ab47aea8bac582
	**SHA256** 3d3f7cc7876ce79697038c620ba723d785be26a18a9734ea10a31fae63992b65

**Torrent**
```text
magnet:?xt=urn:btih:6d749325cbdcf1fc044483fb0d53c233b60735dc&dn=yiffy-e13.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
**Web**
```text
https://iwiftp.yerf.org/Furry/Software/Stable%20Diffusion%20Furry%20Finetune%20Models/Finetune%20models/yiffy-e13.ckpt
```

## SnackBar-General-V1-E11 [16d5a5c7]
>Ce modèle a été entraîné avec la diffusion waifu en utilisant les 300 000 meilleures images sur e621 avec certain tags. Utilisez les tags e621 dans les requêtes avec des soulignements. Vous pourriez obtenir de meilleurs résultats si vous les ordonnez comme suit :
`[safe/questionable/explicit]_content [artist] [copyrights] [personnage] [espèce] [general]`
>Pour de meilleurs résultats, réglez 'Stop At last layers of CLIP model' dans les paramètres de l'interface web sur 2..
>Echantillons d'images:
> https://media.discordapp.net/attachments/528278867572424710/1036765895797788682/09785-69750433-safe_content_renamon.png
> https://media.discordapp.net/attachments/528278867572424710/1036765867645603870/09627-1184405578-safe_content_lucario_anthro_male_solo_cute_clothing_adorable.png

**Torrent**
```text
magnet:?xt=urn:btih:3e18a1d0b988f66c23a472e556ae15fed150d5cc&dn=snackbar-general-v1-e11-pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

**Web**
```text
https://pixeldrain.com/u/uMb3Bnw6
```

## Furry
### Furry_epoch4.ckpt [323f8dd8]
Entraîné sur 300k images de e621. 
Tags: `https://mega.nz/file/co0UlQ5Z#vERcoYTWGJguTsXmysbLq1NL_xBS8txQhVvPI5E3QKE` or `https://pixeldrain.com/u/FQwRjyyk`

!!! note
	**MD5** f8ef45a295ef4966682f6e8fc2c6830d
	**SHA256** 4160c57f98f1727f5a52cba8c844656fc9061311f9c37daf45e8e0ebe913c987

**Torrent**
```text
magnet:?xt=urn:btih:a9635389ae4c5583b0cc76ec8f6dce35438b3016&dn=furry_epoch4.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
**Web**
```text
https://pixeldrain.com/u/dtYiYN7g
```
**Web**
```text
https://iwiftp.yerf.org/Furry/Software/Stable%20Diffusion%20Furry%20Finetune%20Models/Finetune%20models/furry_epoch4.ckpt
```
### Furry_epoch1.ckpt [0c891127]
Entraîné sur 300k images de e621. 
Tags: `https://mega.nz/file/co0UlQ5Z#vERcoYTWGJguTsXmysbLq1NL_xBS8txQhVvPI5E3QKE` or `https://pixeldrain.com/u/FQwRjyyk`

**Torrent**
```
magnet:?xt=urn:btih:d62bc9a088b206565005cab915a58fd26da1802e&dn=furry_epoch1.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
**Web**
```text
https://iwiftp.yerf.org/Furry/Software/Stable%20Diffusion%20Furry%20Finetune%20Models/Finetune%20models/furry_epoch1.ckpt
```

### Furry_epoch0.ckpt [8c19ee5a]
Entraîné sur 300k images de e621.
Tags: `https://mega.nz/file/co0UlQ5Z#vERcoYTWGJguTsXmysbLq1NL_xBS8txQhVvPI5E3QKE` or `https://pixeldrain.com/u/FQwRjyyk`

**Web**
```text
https://iwiftp.yerf.org/Furry/Software/Stable%20Diffusion%20Furry%20Finetune%20Models/Finetune%20models/furry_epoch0_ckpt
```


## Zack3D_Kinky-v1.ckpt [1a75d5c6]
Il s'agit d'un modèle NSFW formé à partir de SD v1.4, idéal pour le furry général mais spécialisé dans le côté pervers de l'art tel que
Transformation, latex, tentacules, slime, zoophilie, bondage, et beaucoup d'autres choses dont je ne me souviens pas. Utilise plus de 100k images, toutes filtrées esthétiquement. 
La requête est constituée de balises d'E621, donc avec des traits de soulignement (underscores).

!!! note
	**SHA1** 9f54dcdbdb2e4c969a9d1b70ba34f02227b8ce10
	**MD5** aa944b1ecdaac60113027a0fdcda4f1b
	**SHA256** 3FD17AF06C610990A957A801A1831F88EF5E45D9C3C8E551557B8B9C6C1FC2D5

**Torrent**
```text
magnet:?xt=urn:btih:807a71d3ed3f887e41c492cf24fbd3c6f5a81534&dn=Zack3D_Kinky-v1.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2fopen.tracker.cl%3a1337%2fannounce
```
**Web**
```
https://pixeldrain.com/u/DEocAHsx
```

## Anal Vore AVHumanFurryPony7.ckpt [68e2e32d]
Anal vore. 7 époques en plus à Zack3D_Kinky-v1. 
Tags : `https://mega.nz/file/FmxSnRZa#I4JgsLTiXWoFUoDrQBWfVNnooUUOxqkFHEOPuCE1Hdc`
Example de requête : `https://mega.nz/file/hu5QnJgJ#DFmvHDuEpm7OdDkQD_wIRrCo7x8j3lqYkhC4aS-egvs`

**Web**
```text
https://mega.nz/file/4jR2WAIZ#p0A7eorXgI_ywC06zQiLcPwE3QdStEa86wwEdrDIE7A
```

## R34
### r34_e4.ckpt [6e45cf2c]
Cinquième époque.
Tags : `https://mega.nz/file/4sFn0AxQ#pHDvIE5eKJC3NSNpgdEw-ve0WSlWjtsTvfpe7YaYh1Q`

**Torrent**
```text
magnet:?xt=urn:btih:ed9f0e3f849d7119107ef4e072c6abeb129e1a51&dn=r34_e4.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2fpublic.tracker.vraphim.com%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/yJgDUCzA#zOD2yeE6QLBqPEjEpIi2b4FWOlb64yVUveOd_eW6teI
```

### r34_e3.ckpt [57ea6f43]
>r34 e3 (quatrième époque). Même donnée d'entraînement, ajout de "by" aux tags des artistes, la reconnaissance des artistes s'est un peu améliorée (elle peut encore être améliorée), la reconnaissance des caractères s'est beaucoup améliorée.

**Web**
```text
https://mega.nz/file/WVggzbRa#4-pcaLeLN02bdvzUwCn2QQKoLxCC7copAJHYyG4aOB0
```

### r34_e2.ckpt [778b68b1]
>Troisième époque, encore une belle amélioration par rapport à la précédente. Les données d'entraînements ont augmenté de ~3K images, avec 5K enlevées et 8K ajoutées.

**Torrent**
```text
magnet:?xt=urn:btih:ef93096b839a54bff77c585b1ba2518bc264d0b4&dn=r34_e2.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.filemail.com%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/3JQ13JTZ#sdByZl4rXp7uxxIDutGs5GHg5RkGZfLrTA5wQEfkDJE
```

### r34_e1.ckpt [f9000e4e]
>J'ai balancé environ 30K images et j'en ai rajouté 20K provenant de bons artistes, et j'ai baissé le taux d''apprentissage' de manière significative pour refléter la taille de mon lot, je pense qu'il s'est amélioré de manière significative mais je n'ai pas sauvegardé les invites/réglages d'epoch0 pour le comparer.

!!! note
	**SHA256** 6ca821f381687796052aa027f60a2dff1aeb001a0c4a2e1f1a1fef4fe89e809a

**Torrent**
```text
magnet:?xt=urn:btih:aa575688ad99cf4bf8e5ee8ab026de9ef61f2d19&dn=r34_e1.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt.oiyo.tk%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce
```

**Web**
```text
https://mega.nz/file/OdxnkAaQ#-9VBG4jyPqlQIcRRnojtNWvhdJQFbmmfGU9I2QGWV5o
```


### r34_150k_epoch0.ckpt  [7dc34283] [7c3721c3]
Entraîné sur 150K images de `rule34.xxx`. 1 époque.
Tags: `https://mega.nz/file/UhsWjBCI#XOvN31pil7_g9YgtzpchpJwyOnVHxrE-dlhxBLYku0I`
!!! note
	r34_150k_epoch0-pruned-fp16.ckpt **[7dc34283]** | **SHA256** f8c7da0d6a98ebd641cb67c24f33510e2c86d1dfa104c10279c7e5cb49d535fd
	r34_150k_epoch0.ckpt **[7c3721c3]** | **SHA256** de2ad17e4af8871cdd42bd03de4faf28983c0ef8702323bbc3d382510d367a91

**r34_150k_epoch0-pruned-fp16.ckpt [7dc34283] Torrent**
```text
magnet:?xt=urn:btih:a8f34e4f7f0bc5298516082b41b5e920b8cc483c&dn=r34_150k_epoch0-pruned-fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.0x.tf%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt2.archive.org%3a6969%2fannounce
```

**r34_150k_epoch0.ckpt [7c3721c3] Torrent**
```text
magnet:?xt=urn:btih:PVTASWQ5GXC4NXGKAZU5LVXJZX3PR6R2&dn=r34_150k_epoch0.ckpt&xl=11142098457&tr=udp%3A%2F%2Ftracker.pomf.se%3A80%2Fannounce
```

**r34_150k_epoch0.ckpt [7c3721c3] Web**
```text
https://mega.nz/file/fdpVCZqB#C3G1_Qs9K65oh_9RHIJoiO1eE5Ns07mvqVdVYXq9uoo
```

##Gape
### gape60 [25396b85]
60ème époque (120k Étapes)
Modèle de béance/insertion large/autre formé sur danbooru, gelbooru, sankaku : tout de `gaping anus`, `gaping pussy`, `large insertion`, `huge dildo`, `fisting`, `prolapse`, `all way through` + quelques autres.
Info https://rentry.org/gapemodel

**Torrent**
```text
magnet:?xt=urn:btih:b892f6e7e6e287ca2ea59d2c575fdf5ae1f82d92&dn=gape60.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.zerobytes.xyz%3A1337%2Fannounce&tr=udp%3A%2F%2Ftracker.zemoj.com%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.pomf.se%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.monitorit4.me%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.moeking.me%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

**Web**
```text
https://mega.nz/file/38RxVBiK#ocmQb_ahp078Imfcvc5SrAvJePOqvhNVBKfZIIeP74Q
```

### gape22 [d69e1a23]
Probablement la 22ème époque.
Modèle de gaping entraîné sur les données de sankaku et e621 : tous les `gaping_anus`, `gaping_pussy`, `large_penetration`, `fisting`, `prolapse`, etc (quelques trucs omis). basé sur yiffy15.
Info https://rentry.org/gapemodel
**Web**
```text
https://mega.nz/file/29hmjYaA#GDdvgwJKKNFLMiaYA2RpuxFk7ormSrS32c05-s3JJCo
```
**Torrent**
```text
magnet:?xt=urn:btih:9d7aabfbac88940a0f93b076f3ce30bb4b06b7e8&dn=gape22_yiffy15.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.pomf.se%3A80%2Fannounce&tr=udp%3A%2F%2Fp4p.arenabg.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Fopen.demonii.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fmovies.zsw.ca%3A6969%2Fannounce&tr=udp%3A%2F%2Fipv4.tracker.harry.lu%3A80%2Fannounce&tr=udp%3A%2F%2Fexplodie.org%3A6969%2Fannounce&tr=udp%3A%2F%2Fexodus.desync.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fbt.oiyo.tk%3A6969%2Fannounce&tr=https%3A%2F%2Ftracker.nanoha.org%3A443%2Fannounce&tr=https%3A%2F%2Ftracker.lilithraws.org%3A443%2Fannounce&tr=https%3A%2F%2Ftr.burnabyhighstar.com%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

### gape18
Probablement la 18ème époque.
Modèle de gaping entraîné sur les données de sankaku et e621 : tous les `gaping_anus`, `gaping_pussy`, `large_penetration`, `fisting`, `prolapse`, etc (quelques trucs omis). basé sur yiffy15.
Info https://rentry.org/gapemodel
**Torrent**
```text
magnet:?xt=urn:btih:9c0fac3978e650e0fa18c18ee74dc35479774c08&dn=gape18_yiffy15.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.pomf.se%3A80%2Fannounce&tr=udp%3A%2F%2Fp4p.arenabg.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Fopen.demonii.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fmovies.zsw.ca%3A6969%2Fannounce&tr=udp%3A%2F%2Fipv4.tracker.harry.lu%3A80%2Fannounce&tr=udp%3A%2F%2Fexplodie.org%3A6969%2Fannounce&tr=udp%3A%2F%2Fexodus.desync.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fbt.oiyo.tk%3A6969%2Fannounce&tr=https%3A%2F%2Ftracker.nanoha.org%3A443%2Fannounce&tr=https%3A%2F%2Ftracker.lilithraws.org%3A443%2Fannounce&tr=https%3A%2F%2Ftr.burnabyhighstar.com%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

## sd-wikiart-v2.ckpt. [8f32b8df]
>sd-wikiart-v2 est un modèle de Stable Diffusion qui a été affiné avec des données d'entraînements provenant de wikiart pour générer des images artistiques dans différents styles et genres.
>Le modèle actuel a été affiné avec un taux d'apprentissage de 1e-05 pour 1 époch sur 81K paires texte-image du jeu de données wikiart. Seules les couches d'attention du modèle sont ajustées. Le modèle peut générer des images artistiques avec des requêtes spécifiques mais conserve la plupart de ses connaissances antérieures.
Info https://huggingface.co/valhalla/sd-wikiart-v2

**Torrent**
```text
magnet:?xt=urn:btih:474493890c2532031fefc8ba3c44d8fed61f287a&dn=sd-wikiart-v2.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

## Pachu-Diffusion [54d5d199]
>Ce modèle Stable Diffusion v1.5 affiné a été entraîné pendant 8000 itérations sur les travaux de Pachu M. Torres. L'entraînement a été effectué à l'aide de ShivamShrirao/diffusers avec une précision totale, une perte de conservation préalable, la fonction train-text-encoder et le nouveau VAE 1.5 MSE de Stability AI. Un total de 4120 images de régularisation/classe a été utilisé. Les images de régularisation ont été générées en utilisant la requête "artwork style", 50 étapes DDIM et un CFG de 7.
>Utilisez les mot-clés `pachu artwork style` dans vos invites pour l'activer.
Dépôt https://huggingface.co/ProGamerGov/Pachu-Diffusion

**Web**
```text
https://huggingface.co/ProGamerGov/Pachu-Diffusion/resolve/main/pachu_artwork_style_v1_iter8000.ckpt
```

## TestFurry_5.ckpt [b1f1855e]
>Petit modèle, entraîné avec 40k image en données d'entraînement mélangées avec des animaux à fourrure et des humains (~15k humains et 25k animaux à fourrure). Pas de censure, pas de bulles, pas de bandes dessinées. Tags mixtes Gelbooru/e621.  5 époques (pas encore terminées mais peut donner de bons résultats) NSFW !
>Recommandation de l'ordre des tags - taper (furry if furry), personnage, source, auteur, race, sexe, autres tags de description.
>Entraîné jusqu'à 225 tags, avec l'avant-dernière couche (CLIP 2 dans l'interface web d'AUTO).


**Torrent**
```text
magnet:?xt=urn:btih:0c0bbbf3182305d6e7e4707e566f2c60cf4e890f&dn=TestFurry_5.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

## cookie_sd_pony_run_a12 [2ce7dcf5] [67ff5385]
>C'est le modèle "Cookie A12".
>Entraîné sur 300k images Derpibooru avec un score>100 pour 2172700 étapes.

>Nouvelles fonctionnalités ;
>- VAE amélioré
>- Amélioration de l'encodeur de texte CLIP
>- Générer des images à n'importe quel taille
>- Générer n'importe quel artiste ou OC derpibooru avec une grande précision.
>- Générer de l'Anthro, de l'Humanisé, de l'EqG, du Dragon, tout ce qui existe dans le derpibooru.
>- Génère des résultats de haute qualité sans "mots magiques" (https://u.smutty.horse/mjzakocpxtf.png)
>- Pas de tags ou de données censurés, NSFW+SFW.

>Note:
>Laissez le saut de CLIP désactivé. Le CLIP amélioré se brisera si vous sautez des couches.

>Samples:
>https://u.smutty.horse/mjzakocpxtf.png
>https://drive.google.com/drive/folders/1nOZKI2iXpzjusvlCsZxF7NxKZYjvKOWE?usp=share_link

**Torrent float16**
```text
magnet:?xt=urn:btih:54d2d1d8f36d9ee192a3b84e6e6c0ddb68f4e891&dn=cookie_sd_pony_run_a12_datasetv5_300k_imgs_fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web float16**
```text
https://sexy.canine.wf/file/furry-ckpt/cookie_sd_pony_run_a12_datasetv5_300k_imgs_fp16.ckpt
```

**Web float32**
```text
https://sexy.canine.wf/file/furry-ckpt/cookie_sd_pony_run_a12_datasetv5_300k_imgs_fp32.ckpt
```

**Web**
```text
https://mega.nz/folder/rIZwEaLZ#AwCpM7Zhu8TFgISQhAE8tw/folder/LRxjWBaJ
```

## pony-diffusion

### pony-diffusion-v2 [5bdc40aa]
>pony-diffusion est un modèle de diffusion latent texte-image qui a été conditionné à des images SFW et NSFW de haute qualité de poneys et de furry grâce à un réglage fin.
>AVERTISSEMENT : Comparé à la version 1 de ce modèle, la version 2 est beaucoup plus capable de produire du contenu NSFW. Il est donc recommandé d'utiliser la balise 'safe' dans l'invite en combinaison avec l'invite négative pour les caractéristiques de l'image que vous souhaitez supprimer. Le modèle 2 a également un léger biais 3d. Les invites négatives comme '3d' ou 'sfm' doivent être utilisées pour mieux correspondre aux résultats de la version 1.

Dépôt https://huggingface.co/AstraliteHeart/pony-diffusion-v2

**Web**
```text
https://mega.nz/file/Va0Q0B4L#QAkbI2v0CnPkjMkK9IIJb2RZTegooQ8s6EpSm1S4CDk
```

### pony-diffusion-v1 [9070b574]
Ajusté en plus d'un point de contrôle précoce de Waifu Diffusion avec un taux d'apprentissage de 5.0e-6 pour 4 époques sur environ 80k pair text-image de poney (en utilisant les tags de derpibooru) qui ont tous un score supérieur à 500 et appartiennent aux catégories `safe` ou `suggestive`.
Info `https://huggingface.co/AstraliteHeart/pony-diffusion`
!!! note
	**SHA256** 507dd62efcb01db04aeb1b68eb28912d1fe692c04637e463cc43e68752729627

**Web**
```text
https://mega.nz/file/ZT1xEKgC#Xxir5udMmU_mKaRZAbBkF247Yk7DqCr01V0pDzSlYI0
```

## mio-wd-v1.2-e24-ex-ad [125f9ece]
Petit modèle entraîné sur 500 images (aprox) sur Mio (nichijou). 24 époques. 
Info: `https://huggingface.co/chavinlo/mio-naganohara-waifu-diffusion`

!!! note
	**SHA256** 4744ed99c119096fa144df9aa8c869d933e3b6a239786462c5cc399ff1d78698

**Torrent**
```text
magnet:?xt=urn:btih:MS2YZ762U2IJZBG2GSEADRAA5KAOQCVQ&dn=mio-wd-v1.2-e24-ex-ad&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce
```
**Web**
```text
https://huggingface.co/chavinlo/mio-naganohara-waifu-diffusion/resolve/main/epoch%3D000023-pruned.ckpt
```
## fubuki-ld-v1-e13-ex-ad [2af6d20f]
Modèle moyen entraîné sur 5 000 images de Fubuki provenant de Hololive. 13 époques, taillé, 3.8GB, incluant les échantillons.

**Torrent**
```text
magnet:?xt=urn:btih:D37QX2URIZ7QWMBCCLMZG6THZF5GLDB2&dn=fubuki-ld-v1-e13-ex-ad&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce
```
## asuka-ld-v1-e4-ex-ad [87074080]
Petit modèle entraîné sur 17 000 images d'Asuka d'Evangelion. 4 époques, taillé, 3.6GB, n'inclut pas les échantillons..

**Torrent**
```text
magnet:?xt=urn:btih:4GBOL6SMWLZWT2CBUWVMSF5NCGD47CGZ&dn=asuka-ld-v1-e4-ex-ad&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce
```
## tomoko-kuroki-ld-v1-e20-ex-ad [6095e7ab]
Petit modèle formé sur un nombre inconnu d'images de Tomoko Kuroki. 20 époques, taillé, 3.5GB, n'inclut pas les échantillons.

**Torrent**
```text
magnet:?xt=urn:btih:3OFJJ76Y2K4W47RJ7EXFKI3BAZPPVTQM&dn=tomoko-kuroki-ld-v1-e20-ex-ad&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce
```

## Merges

### BerryMix [19810fe6]
Si vous avez déjà été mariné de manière satisfaisante, vous pouvez peut-être essayer d'être mixé ensuite.
Info https://rentry.org/berrymix

**Torrent**
```text
magnet:?xt=urn:btih:e975132162842e0b3e96948f8bffcb66071ecc97&dn=berry_mix.ckpt&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### FreckleGuy's mix
>Une fusion avec un modèle d'anime et trinart avec une ancienne version de l'interface web d'automatic1111.

**Torrent**
```text
magnet:?xt=urn:btih:1968aed673e402127b6abab535059f622f93b989&dn=freckle_mix.ckpt&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### HassansBlend
>L'accent est toujours mis sur le photoréalisme, mais il est possible d'ajouter des poses, des scènes de sexe, etc. Exemples inclus sur la page avec les images et les métadonnées à copier.. 
>https://rentry.org/sdhassan

>View sample albums from this model
>Other: https://imgur.com/a/c8VEbGJ
>Tentacles: https://imgur.com/a/l5hg69w
>Retro realistic: https://imgur.com/a/N5W5Rtn
>Normal nudes: https://imgur.com/a/14jAPBN
>Hardcore: https://imgur.com/a/WZJSvha 


### MMDv1-18
Une fusion d'un nombre exorbitant de modèles.
Info https://huggingface.co/ShinCore/MMDv1-18

**Web**
```text
https://huggingface.co/ShinCore/MMDv1-18/resolve/main/MMD%20V1-18%20MODEL%20MERGE%20(TONED%20DOWN)%20ALPHA.ckpt
```

**Web**
```text
https://huggingface.co/ShinCore/MMDv1-18/resolve/main/MMD%20V1-18%20MODEL%20MERGE%20ALPHA.ckpt
```

 ### samdoartsultmerge.ckpt [8687d7a5]
>Voici un modèle fusionné utilisant l'original, le modèle de chewtoy, et le modèle de Chris(orginalcode) 
Utilisez le mot-clé `samdoesarts` dans la requête.
Dépôt https://huggingface.co/jinofcoolnes/sammod/tree/main

**Torrent**
```text
magnet:?xt=urn:btih:7357defc010aa7e05e60f29149ad20b09aec78ad&dn=samdoartsultmerge.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://huggingface.co/jinofcoolnes/sammod/resolve/main/samdoartsultmerge.ckpt
```


### 70gg30LD70k.ckpt [402dc090]
Une fusion entre gg1342_testrun1_pruned.ckpt et LD-70k-1e-pruned.ckpt, ratio 70-30.
Cette liste n'est pas destinée aux fusions, vous pouvez les faire vous-même avec l'interface Web.
Celle-ci est ici parce qu'elle est référencée (et demandée) très souvent.

**Torrent**
```text
magnet:?xt=urn:btih:5667722f227d1534232182de77730f8aa7ff8212&dn=70gg30LD70k.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.tiny-vps.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fzecircle.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fyahor.ftp.sh%3A6969%2Fannounce&tr=udp%3A%2F%2Fvibe.sleepyinternetfun.xyz%3A1738%2Fannounce&tr=udp%3A%2F%2Fv2.iperson.xyz%3A6969%2Fannounce&tr=udp%3A%2F%2Fuploads.gamecoast.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker2.dler.org%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.theoks.net%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.tcp.exchange%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.swateam.org.uk%3A2710%2Fannounce&tr=udp%3A%2F%2Ftracker.publictracker.xyz%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```
### wd1-2_sd1-4_merged.ckpt [2037c511]
Une fusion entre Waifu Diffusion v1.2 et Stable Diffusion v1.4 au ratio inconnu.
Cette liste n'est pas destinée aux fusions, vous pouvez les faire vous-même avec l'interface Web.
Celle-ci est ici parce qu'elle est référencée (et demandée) très souvent.

**Torrent**
```text
magnet:?xt=urn:btih:a1515e051c61ac55e64a7961da956dc2f5c00e90&dn=wd1-2_sd1-4_merged.ckpt&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2F9.rarbg.com%3A2810%2Fannounce&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A6969%2Fannounce&tr=udp%3A%2F%2Fopentracker.i2p.rocks%3A6969%2Fannounce&tr=https%3A%2F%2Fopentracker.i2p.rocks%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Fopen.stealth.si%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker1.bt.moack.co.kr%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.moeking.me%3A6969%2Fannounce&tr=udp%3A%2F%2Fp4p.arenabg.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fopen.demonii.com%3A1337%2Fannounce&tr=udp%3A%2F%2Fipv4.tracker.harry.lu%3A80%2Fannounce&tr=udp%3A%2F%2Fexplodie.org%3A6969%2Fannounce&tr=udp%3A%2F%2Fexodus.desync.com%3A6969%2Fannounce&tr=https%3A%2F%2Ftracker.nanoha.org%3A443%2Fannounce&tr=https%3A%2F%2Ftracker.lilithraws.org%3A443%2Fannounce&tr=https%3A%2F%2Ftr.burnabyhighstar.com%3A443%2Fannounce&tr=http%3A%2F%2Ftracker.mywaifu.best%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.dler.org%3A6969%2Fannounce&tr=http%3A%2F%2Ftracker.bt4g.com%3A2095%2Fannounce
```

## Dreambooth
### Hiten girl_anime_8k_wallpaper_4k.ckpt [7831dea3]
Utile pour la fusion.
Modèle personnalisé de Dreambooth basé sur l'artiste Hiten. Les données d'entraînement sont 6 images et 34 images de régularisation. Entraîné pendant 4000 étapes.
Utilise le modèle Waifu Diffusion v1.2 comme base.
Pour utiliser le modèle, il suffit d'insérer le nom "Hiten" dans vos invites. Le mot-clé utilisé est 'girl_anime_8k_wallpaper'. Ajoutez le mot-clé après Hiten pour obtenir de meilleurs résultats. EX : "1girl by Hiten girl_anime_8k_wallpaper".

Info: `https://huggingface.co/BumblingOrange/Hiten`
!!! note
	**SHA256** | 59cb5b6f2b7b9373a602d93c802ee841c367db5166a758e8d1d32464c9db1dd5

**Web**
```text
https://huggingface.co/BumblingOrange/Hiten/resolve/main/Hiten%20girl_anime_8k_wallpaper_4k.ckpt
```

### nanachiDB-42imgs-5000steps.ckpt
> nanachi 42 images en plus de yiffy-e15
> Mot-clé: nanachiDB cute_furry_girl
**Web**
```text
https://mega.nz/file/xE9gFQYK#f61_2_OvDSOd4VRW3W9EoLpImwCBf1hauUFhW-iNtRw
```

### rizaDB-54imgs-4500steps.ckpt
> modèle ryza dreambooth (54 images en plus de WD 1.3 beta
> Mot-clé: rizaDB anime_girl (oui, j'ai mal orthographié le nom)
> Attention : il peut générer des résultats nsfw par défaut.
```text
https://mega.nz/file/dc1EDKCa#NTXIyMRCpFfS8BsW3s-VeT950ApRTzE8_aVciLmx1bM
```

### bea-14000-pruned-fp16.ckpt [802a681b]
>Bea (Pokemon S/S) NSFW DreamBooth model
>Données d'entraînement: 24 images de fanarts triées sur le volet et recadrées, 50% SFW, 50% NSFW.
>Étapes: `14000`
>Taux d'apprentissage: `6.5e-07`
>Mot-clé (identifiant + classe): `sks girl`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle.
>Données d'entraînement incluses dans le torrent. 

**Torrent**
```text
magnet:?xt=urn:btih:18c7932594fcf2b67434103d3af422a92ebac625&dn=dreambooth-bea-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
magnet:?xt=urn:btih:18c7932594fcf2b67434103d3af422a92ebac625&dn=dreambooth-bea-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### 2b-10000-pruned-fp16.ckpt [8cf4478f]
>YoRHa 2B (Nier) DreamBooth NSFW
>Données d'entraînement: 18 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Étapes: `10000`
>Taux d'apprentissage: `6.5e-07`
>Mot-clé (identifiant + classe): `sks girl`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..
>Données d'entraînement incluses dans le torrent.

**Torrent**
```text
magnet:?xt=urn:btih:14f5414d184f36c1932f310661eb82e14165931b&dn=dreambooth-2b-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### misato-10000-pruned-fp16.ckpt [c51e4c77]
>Misato Katsuragi (Casual Outfit) (Neon Genesis Evangelion) DreamBooth NSFW
>Données d'entraînement: 21 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Étapes: `10000`
>Taux d'apprentissage: `6.5e-07` (i think?)
>Base Model: WD 1.3 epoch 8
>Mot-clé (identifiant + classe): `sks katsuragi_misato`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..
>Training images included in torrent.

**Torrent**
```text
magnet:?xt=urn:btih:b4751304bbb04c719233436f92806cf278da74ab&dn=dreambooth-misato-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.skyts.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### hilda-v2-8000-pruned-fp16.ckpt [d2c8eef1]
>Hilda (Pokemon) DreamBooth NSFW
>Données d'entraînement: 24 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Étapes: `8000`
>Base Model: WD 1.3 epoch 8
>Mot-clé (identifiant + classe): `sks hilda_(pokemon)`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..

**Torrent**
```text
magnet:?xt=urn:btih:37c7fc96d630dafc14c3e46526507ca66e760afc&dn=dreambooth-hilda-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce
```

### nagatoro-4000-pruned-fp16.ckpt [8c5ff341]
>Nagatoro DreamBooth NSFW
>Données d'entraînement: 16 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Taux d'apprentissage: `2.9e-07` (imgs*0.18)
>Étapes: `4000`
>Mot-clé (identifiant + classe): `sks nagatoro_hayase`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..

**Torrent**
```text
magnet:?xt=urn:btih:fc70fc0613601df43ffc7e7921250debf925db7b&dn=dreambooth-nagatoro-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### gawr_gura-4000-pruned-fp16.ckpt [cbd4da60]
>Gawr Gura (Hololive) DreamBooth Model NSFW
>Données d'entraînement: 24 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Taux d'apprentissage: `4.3e-07` (imgs*0.18)
>Étapes: `4000`
>Mot-clé (identifiant + classe): `sks gawr_gura`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..

**Torrent**
```text
magnet:?xt=urn:btih:9200076f1cc41258a8069a6c44e948f5f6cd515b&dn=dreambooth-gura-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### mori_calliope-4000-pruned-fp16.ckpt [6cb50083]
>Calliope Mori (Hololive) DreamBooth Model NSFW
>Données d'entraînement: 14 hand-picked, hand-cropped fanart images, roughly half of them NSFW
>Taux d'apprentissage: `2.5e-07` (imgs*0.18)
>Étapes: `4000`
>Mot-clé (identifiant + classe): `sks mori_calliope`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..

**Torrent**
```text
magnet:?xt=urn:btih:969cabc39c8363aeec824f5530bb0749b6452621&dn=dreambooth-calli-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce
```

### towa-4000-pruned-fp16.ckpt [d578d3c1]
>Tokoyami Towa (Hololive) DreamBooth Model NSFW
>Données d'entraînement: 15 fanart images, roughly half of them NSFW
>Taux d'apprentissage: `2.7e-07` (imgs*0.18)
>Étapes: `4000`
>Class: `tokoyami_towa`
>Toutes les images d'entraînement dépeignent le personnage comme ayant au moins 18 ans - Je ne suis pas responsable des résultats de la génération utilisant ce fichier modèle..

**Torrent**
```text
magnet:?xt=urn:btih:207fef192613543d219a861b37e04c138a7a8597&dn=dreambooth-towa-nsfw&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce
```

### Gawr_Gura_450img.ckpt [e7883abf]
Basé sur Waifu Diffusion v1.2
Modèle personnalisé de Dreambooth basé sur la ressemblance avec la V-tubeuses Gawr Gura de Hololive. 450 images d'entraînement et  900 images de régularisation. Entraîné pour 3000 étapes.
Pour utiliser le modèle, il suffit d'insérer le mot-clé 'Gawr Gura' dans vos requêtes.
Dépôt: https://huggingface.co/BumblingOrange/GuraLv400
**Web**
```text
https://huggingface.co/BumblingOrange/GuraLv400/resolve/main/Gawr_Gura_450img.ckpt
```

### Emilia_CustReg_3k.ckpt [2660cf9a]
Basé sur Waifu Diffusion v1.2
Modèle personnalisé de Dreambooth basé sur la ressemblance d'Emilia de Re:Zero. 16 images d'entraînement et 11 images de régularisation. Entraîné pour 3000 étapes.
Pour utiliser le modèle, il suffit d'insérer le mot-clé 'Emilia' dans vos requêtes. Le mot-clé utilisé est 'white_hair_girl_violet_eyes'. Ajoutez le mot-clé après Emilia pour obtenir de meilleurs résultats. EX : "A photo Of Emilia white_hair_girl_violet_eyes".
Dépôt: https://huggingface.co/BumblingOrange/Emilia
```text
https://huggingface.co/BumblingOrange/Emilia/resolve/main/Emilia_CustReg_3k.ckpt
```

### Star Fox Krystal 
Entraîné sur 90 images de fan art pour 2500 étapes en plus de yiffy18. Les images réglementaires étaient un mélange homogène d'environ 200 générations de basse qualité de 4 espèces d'anthro communes avec une répartition des genres (une nouvelle idée que j'espère expliquer dans un prochain guide).
Le texte suivant DOIT être utilisé dans votre requête, sinon il ne servira à rien !
Mot-clé `thatbluefoxfromstarfoxkrystal anthro`

!!! note
	**SHA256** 13adea314c8653de231c454fa9e911d80ab04640caf30d752802c9b201d4c243

**Web**
```text
https://mega.nz/file/6x4zETbS#E22vQhjjK9CmBQXkzZgNFkIF4-TWGsyE0GRFP7pk-kE
```

**Web**
```text
https://pixeldrain.com/u/EH2mvEux
```

### StudioGhibli [10c6aa67]
Mot-clé `studio_ghibli_anime_style style`
Dépôt https://huggingface.co/IShallRiseAgain/StudioGhibli

**Web**
```text
https://huggingface.co/IShallRiseAgain/StudioGhibli/resolve/main/StudioGhibliV4.ckpt
```

### Arcane Vi vimod.ckpt [e02601f3]
Utilisez `sks vi` ou `vi` dans les requêtes.
!!! note
	**SHA256** 2db4e932c1d7e3ac4b8c9d94d9b9c98487941f3b6c5617973351f898374d7aae

**Web**
```text
https://huggingface.co/jinofcoolnes/VImodel/resolve/main/vimod.ckpt
```

### irlhyperbreasts_9k.ckpt [5ce8b6ec]
Aperçu https://i.imgur.com/GIsZKH4.jpg
Mot-clé `hyperbreasts`

**Torrent**
```text
magnet:?xt=urn:btih:350e93a86f6d08724a03e8175e1947fc7e123e4c&dn=irlhyperbreasts_9k.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://drive.google.com/drive/folders/1ghNQ5kEXlnuuRTGMobfyr81W7N6bURMi
```

### hyperass v1
>Hyper Ass Dreambooth model [NSFW] v1
>-800 images
>-10,000 étapes
>-Taux d'apprentissage 5e-6
C'est le dernier modèle du trio hyper BBB (seins, fesses, ventre), et le dernier que j'ai prévu pour l'instant.

Mot-clé `hyperass`

**Torrent**
```text
magnet:?xt=urn:btih:cba3a8020f7b69695301412b58c35f7c17826c39&dn=dreambooth-hyper-ass&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce%20&tr=http%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce%20&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce
```

### hyperbreasts: v3
>Hyper Breasts Dreambooth model [NSFW] v3
>-1.2k images
>-10,000 étapes
>-Taux d'apprentissage 1e-6
>Celle-ci a été entraînée sur moins d'images en général, mais de meilleure qualité et de plus grande taille.  Résultats plus gros la v2.
>Mot-clé `hyperbreasts`

**Torrent**
```text
magnet:?xt=urn:btih:697ac36f1539ddd88b84a6237cf25ed11388bf43&dn=dreambooth-hyper-breasts-v3&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=http%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce%20&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce
```

### hyperpreg: v2 (réalisme et anime)
>Mot-clé `hyperpreg`

**Torrent**
```text
magnet:?xt=urn:btih:b90176670760eee75156197644a461d61a64990d&dn=dreambooth-hyper-preg-v2&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=http%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce%20&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce
```

### hyperpreg: v1.1 (realism)
>The difference between v1 and v1.1 is that v1.1 focuses on even larger belly sizes, with a smaller more refined dataset
>1k images
>50,000 étapes
>Taux d'apprentissage 1e-6 cosine
>modèle de base sd-v1-5
>Mot-clé: `hyperpreg`

**Torrent**
```text
magnet:?xt=urn:btih:ee0959baa76604ebe17dd88b58bff32145cd9fb6&dn=dreambooth-hyper-preg-v1.1&tr=udp%3a%2f%2f93.158.213.92%3a1337%2fannounce%20&tr=udp%3a%2f%2f151.80.120.113%3a2810%2fannounce%20&tr=udp%3a%2f%2f91.216.110.53%3a451%2fannounce
```

### hyperpreg: v1 (realism)
hyper_preg_2.7k-lr5e-7-15ks-fp16.ckpt
Hyper Preg Dreambooth model NSFW
>2.7k images
>15,000 étapes
>Taux d'apprentissage `5e-7`
>modèle de base `sd-v1-4`
>Mot-clé `hyperpreg`
>Classe `round belly`

**Torrent**
```text
magnet:?xt=urn:btih:wlu5ltx6nf637e3au2zk4r2o2y6oi3ol&dn=dreambooth-hyper-preg&xl=2133006686&fc=2
```

### Kurisu
>Kurisu NSFW 
>Mot-clé (identifiant + classe): `sks kurisu`
>Étapes - 4000
>Images - 16 avec quelques NSFW

**Torrent**
```text
magnet:?xt=urn:btih:2380decbde8386095241729e38cb6bbead93c0c2&dn=kurisu_final_pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### Kurisu Visual Novel Official artist (Huke) [eda12c8e]
kurisu_vn_final_4k_pruned.ckpt
>Peut faire du NSFW
>Mot-clé (identifiant + classe): `sks kurisu_vn`
>Étapes - 4000 (tried 6k and it was overfitted)
>12 images SFW de l'artiste du Steins;Gate, Huke
>Note rapide sur ce modèle, si vous souhaitez générer différents personnages et utiliser "highres fix" dans l'interface utilisateur d'Auto, réglez la puissance de reduction du son à un niveau élevé pour éviter les artefacts dus au style artistique, de préférence >0.85. Ajustez le CFG si nécessaire, 5-7 est un bon début. 

**Torrent**
```text
magnet:?xt=urn:btih:04c933900719a5833fbe07581ffa6c5114d3b7b6&dn=kurisu_vn_final_4k_pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### dreambooth_lain_girl.ckpt [e7629bf8]
Mot-clé `lain girl`

**Torrent**
```text
magnet:?xt=urn:btih:8fb4f0fe0ce052b5d6f0d1221a69f04aa2ea406d&dn=dreambooth_lain_girl.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/VK0U0ALD#YDfGgOu8rquuR5FbFxmzKD5hzxO1iF0YQafN0ipw-Ck
```

### Gyokai/onono imoko 
>Le meilleur modèle pour la cohérence est gyokaidreambooth_1000.
>gyokaidreambooth_2000 commence à ignorer les requêtes mais est très cohérent dans son style.
>Utilisez le mot-clé `gyokai` en dessous de la classe `1girl`
>Exemple : `illustration of gyokai 1girl`
>Fonctionne bien avec `multicolored hair, colorful, halftone, polka dot`
>Astuce: le modèle fonctionne bien lorsqu'il est associé à gyokai-zen HN à 0,85 de force de reduction de bruit
>gyokai-zen: `https://mega.nz/file/67AUDQ4K#8n4bzcxGGUgaAVy7wLXvVib0jhVjt2wPS-jsoCxcCus `

**Web**
```text
https://mega.nz/folder/HzYT1T7L#H9TWVVYowA0cX8Eh6x_H3g
```

### futacum_r34.ckpt
>Comprends 'futa' et 'cumming' ; basé sur r34_e4, aspect cartoonesque

**Web**
```text
https://drive.google.com/file/d/1cbCm5KJQgIeD5Om6tkbwGtqHG6Pa0gBz/view
```

### futanari_v2_e3_s10000.ckpt
>Comprends "futanari" ; très anime-ish

**Web**
```text
https://drive.google.com/file/d/1PhCzlHf-PpxynzYcQPQ3INk5tfBL9Bb1/view?usp=sharing
```

### Futa_step_3200_10_27.ckpt
>Comprends "futa" ; basé sur SD1.5, réaliste

**Web**
```text
https://drive.google.com/file/d/1-3xKhdXwlbPIXt8OjBvlamTiQUGza0hn/view
```

### Wayne Barlowe [e0c27550]
Entraîné sur 31 œuvres SFW de l'artiste Wayne Barlowe
>Trigger Phrase: `DBWayneBarlowe`
>Étapes: `4000`
>Images: 
>Model: `SD_1.5_pruned`
>Utilisez des requêtes à caractère religieux ou démoniaque

!!! note
	**SHA512**  30fd8065ff69438c0ef96e063490f6ed4a8c7b41ac8433a36e592f285937f34cc45212d7c44846161a3b5f7977d9700778bf8d566beb653e56babf079f570302

**Torrent**
```text
magnet:?xt=urn:btih:a488fb7b0403988df282b422c3d9383e17bbc646&dn=DBWayneBarlowe_JM.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce&tr=https%3a%2f%2ftr.burnabyhighstar.com%3a443%2fannounce
```

**Web**
```text
https://gofile.io/d/VPm7wu
```

**Web**
```text
https://pixeldrain.com/u/9pPL2ikF
```

### Blacked POV blowjob [8467f44f]
dreambooth-bkdbj-4000step-pruned-fp16.ckpt
>Modèle de fellation PDV de Blacked, entraîné pour 4000 étapes sur 70 photos sélectionnées à la main sur blacked.com, entraîné par-dessus le modèle SD 1.4 en utilisant Dreambooth. Mot-clé pour les requêtes : `bkdbj`
>Toutes les images d'entraînement utilisées représentent l'actrice âgée d'au moins 18 ans. Je ne suis pas responsable des résultats de génération obtenus avec ce fichier modèle ou les modèles fusionnés avec celui-ci. Je ne fournirai aucun support ni aucune mise à jour supplémentaire pour ce modèle. C'est un modèle "one trick pony", car il ne peut faire qu'un seul type de scène NSFW. La taille pour la génération qui fonctionne est 512x512 ou jusqu'à 512x640.

**Torrent**
```text
magnet:?xt=urn:btih:836422af364efb784e61a049c1257ea1230509b7&dn=dreambooth-bkdbj-4000step-pruned-fp16.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce
```

**Web**
```text
https://gofile.io/d/aMfZIV
```

### Zeipher F222 Female Nude (better anatomy)
De : https://ai.zeipher.com/
>Tous ces éléments sont encore en cours d'entraînement et non définitifs.
Consultez leur site web/discord pour les téléchargements et les mises à jour éventuelles.

**Torrent F222**
```text
magnet:?xt=urn:btih:GR3IGMJDPJPW3B4WRT5B7SAN7CEBHWSZ&dn=f222&tr=http%3A%2F%2Ftracker.openbittorrent.com%2Fannounce
```
**Web**
```text
https://redirect.ai.zeipher.com/b1t50kc
```
Précédemment F111.
Précédemment F99.

### Zeipher BB (Blowjob images)
From: https://ai.zeipher.com/
>Tous ces éléments sont encore en cours d'entraînement et non définitifs.
Consultez leur site web/discord pour les téléchargements et les mises à jour éventuelles.

### LOKEAN_PUPPYSTYLE_POV.ckpt [e02601f3]
Images photoréalistes de levrette hétéro.
Pour une raison quelconque, partage un hachage court avec Arcane Vi vimod.ckpt et lokean missionnaire. Cela est probablement dû à la façon dont il a été formé - le hachage ne concerne qu'un petit segment proche du début du fichier. Ce sont des modèles différents.
Utilisez `LOKEAN_PUPPYSTYLE_POV` pour l'activer.
Info et Apercu https://rentry.org/kwai

**Web**
```text
https://agencykwai.vip/SD/LOKEAN_PUPPYSTYLE_POV.ckpt
```

### LOKEAN_MISSIONARY_POV.ckpt [e02601f3]
Images photoréalistes de missionnaires hétéro.
Pour une raison quelconque, partage un hachage court avec Arcane Vi vimod.ckpt et lokean puppystyle. Cela est probablement dû à la façon dont il a été formé - le hachage ne concerne qu'un petit segment près du début du fichier. Ce sont des modèles différents.
Utilisez `LOKEAN_MISSIONARY_POV` pour l'activer.
Info and Apercu https://rentry.org/kwai

**Web**
```text
https://agencykwai.vip/SD/LOKEAN_MISSIONARY_POV.ckpt
```

### Reverse cowgirl s4629674.ckpt [e285da0d]
Modèle pornographique de cowgirl inversée photoréalistique
Mot-clé `s4629674`

**Torrent**
```text
magnet:?xt=urn:btih:7dd135c05ce2693fbcaf0e6e2f9975dfa871f745&dn=s4629674.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zemoj.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/Q4tDRTAJ#GvgsNp_AnCuniY_rpdrerDQtVaGO90dQlyB__wT4dMM
```

### elden-ring-diffusion [16d77205]
Il s'agit d'un modèle de Stable Diffusion affiné et entraîné sur l'art du jeu de Elden Ring. 
Utilisez les jetons `elden ring style` dans vos requêtes pour l'effet.
Dépôt https://huggingface.co/nitrosocke/elden-ring-diffusion
Vérifiez le dépôt pour d'éventuelles nouvelles versions.

**Web**
```text
https://huggingface.co/nitrosocke/elden-ring-diffusion/resolve/main/eldenRing-v3-pruned.ckpt
```

### modern-disney-diffusion [ccf3615f]
Il s'agit d'un modèle de Stable Diffusion affiné et entraîné sur des captures d'écran d'un studio d'animation populaire. 
Utilisez les jetons `modern disney style` dans vos requêtes pour l'effet.
Dépôt https://huggingface.co/nitrosocke/mo-di-diffusion
Vérifiez le dépôt pour d'éventuelles nouvelles versions.


**Web**
```text
https://huggingface.co/nitrosocke/mo-di-diffusion/resolve/main/moDi-v1-pruned.ckpt
```

### Arcane-Diffusion [318a302e]
Il s'agit d'un modèle de Stable Diffusion affiné et entraîné à partir d'images de la série télévisée Arcane. 
Utilisez les jetons `arcane style` dans vos requêtes pour l'effet.
Dépôt https://huggingface.co/nitrosocke/Arcane-Diffusion
Vérifiez le dépôt pour d'éventuelles nouvelles versions.

**Web**
```text
https://huggingface.co/nitrosocke/Arcane-Diffusion/resolve/main/arcane-diffusion-v3.ckpt
```

### classic-anim-diffusion [be7ddafc]
Il s'agit d'un modèle de Stable Diffusion affiné et entraîné sur des captures d'écran d'un studio d'animation populaire. 
Utilisez les jetons `classic disney style` dans vos requêtes pour l'effet.
Dépôt https://huggingface.co/nitrosocke/classic-anim-diffusion
Vérifiez le dépôt pour d'éventuelles nouvelles versions.

**Web**
```text
https://huggingface.co/nitrosocke/classic-anim-diffusion/resolve/main/classicAnim-v1.ckpt
```

### bukkake [b4a14787]
bukkake_20_training_images_2020_max_training_steps_woman_class_word.ckpt
SD1.4 + Dreambooth
>Une bonne requête à tester est "portrait of bukkake woman, cute girl, sexy, beautiful girl, photograph, nikon d4, f/2.8, 70mm, topless" 
>Conseil : N'utilisez PAS la restauration de visage si vous essayez de faire txt2img pour obtenir une sorte de sperme réaliste sur un visage réaliste. Les deux modèles de restauration de visage actuellement disponibles (GPFGAN et Codeformer) enlèvent ou ruinent complètement le sperme sur le visage. Espérons qu'un autre développeur proposera un nouveau modèle de restauration de visage qui ignore les liquides sur les visages tout en corrigeant le visage sous-jacent.

**Torrent**
```text
magnet:?xt=urn:btih:90df9e64cd31b686c858b9d50b54efafd6a96984&dn=bukkake_20_training_images_2020_max_training_steps_woman_class_word.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.swateam.org.uk%3a2710%2fannounce&tr=udp%3a%2f%2ftracker.srv00.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.lelux.fi%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce
```

**Web**
```text
https://mega.nz/file/olV0yDrR#-5ShWX92PP6r8wV6jHMHQjOmEGnq82xY6-J048GOf9s
```

### DCAU [2fd6057b]
Un modèle pour l'univers animé de DC. Actuellement, il n'a été entraîné que sur Batman the Animated Series, mais il est prévu d'inclure toutes les séries.
Dépôt (Avec échantillons) https://huggingface.co/IShallRiseAgain/DCAU
Vérifiez le dépôt pour d'éventuelles nouvelles versions.
Mot-clé : `Batman_the_animated_series`.

**Web**
```text
https://huggingface.co/IShallRiseAgain/DCAU/resolve/main/DCAUV1.ckpt
```

### Raven anime.ckpt
>Modèle "anime" - Un modèle plus récent. Il est un peu moins entraîné pour plus de flexibilité. Il peut créer des images plus complexes avec une plus grande stabilité, mais avec un style anime plus prononcé.
>Mot-clé & nom de classe nécessitent plus d'emphase / attention que le modèle 'Western'. Essayer (RavenRoth woman:1.2). Il est un peu sous-entraîné pour plus de flexibilité - il faudra peut-être utiliser la "peau grise" dans la requête. (RavenRoth woman:1.2) avec peau grise.
>Mot-clé & nom de classe: `RavenRoth woman`

**Web**
```text
https://mega.nz/folder/4KlHFQaL#NBqANoiG3E7mz3apn85Q0g
```

### Raven Western.ckpt
>Il s'agit d'un modèle Dreambooth antérieur que j'ai créé. Il a un style plus fidèle au dessin animé Teen Titans et à "Zone art".
>J'ai remarqué que les requêtes négatives peuvent faire en sorte que le modèle s'éloigne d'un style de dessin animé/de zone, alors faites attention aux requêtes négatives si vous trouvez cela...
>Une requête négative: 'lowres, (bad anatomy), bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, jpeg artifacts, >signature, watermark, username, blurry, artist name' ça marche bien.
>Je pense que l'IA considère que le style artistique du trait épais est... 'low quality' et pour que Raven soit 'deformed' ou 'mutant', à cause de la peau grise.
>Essayez de réduire l'échelle CFG (closer to 5) pour plus de flexibilité. Vous pouvez même essayer `[RavenRoth anime_girl]`
>Mot-clé & nom de classe: `RavenRoth anime_girl`

**Web**
```text
https://mega.nz/folder/4KlHFQaL#NBqANoiG3E7mz3apn85Q0g
```

### oshinobu-pruned.ckpt
>Données d'entraînement: 63 images fanart qui ressemblent au design des personnages de l'anime (seulement 3 ou 4 sont NSFW)
>étapes: 9450 (~6 époques)
>Base Model: Waifu Diffusion v1.3
>Mot-clé (identifiant + classe): `illustration of a oshinobu blonde_loli`
>En raison des données d'entraînement utilisées, les images générées conservent son rougissement typique (la plupart du temps)..
>Maintenant, je pense que j'ai utilisé une quantité folle d'étapes et d'images, mais lorsque j'ai entraîné ceci (il y a une semaine), je n'avais aucune idée de ce que je faisais exactement.

**Torrent**
```text
magnet:?xt=urn:btih:bd00f28f19715632b62eae8660cbffb14d606c20&dn=oshinobu-pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### henreader_final_pruned.ckpt [e5803978]
Mot-clé (identifiant + classe): `sks henreader_db`

**Torrent**
```text
magnet:?xt=urn:btih:2ea2f859da0dd0b48d39f7296bee20434b40e92b&dn=henreader_final_pruned.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### oshino_shinobu_final_pruned.ckpt
Mot-clé (identifiant + classe): `photo of a sks oshino_shinobu_db`

**Torrent**
```text
magnet:?xt=urn:btih:ed81724913f0cf899c6eed4a896ff1480b243761&dn=oshino_shinobu_final_pruned.ckpt&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2fpublic.tracker.vraphim.com%3a6969%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.openbittorrent.com%3a6969%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce
```

### Latex's Abandoned Style
>Ce style est basé sur l'artiste Latex, un artiste qui avait l'habitude de faire du latex mais qui un jour a complètement changé de style artistique, a effacé ses vieilles images de la plupart des sites internet et a cessé de faire du latex (mais a gardé le nom ironiquement). Heureusement, j'ai une décharge de leur ancien art, donc enfin il ya un moyen de refaire l'un de mes styles préférés de latex / goo art. J'ai attaché quelques générations imparfaites rapides à jeter un oeil dessus. Je suis sûr que vous allez me surpasser très très rapidement.
>Mot-clé : `latex_artist art`

**Torrent**
```text
magnet:?xt=urn:btih:d14d31cbddbd3a1be01d4e89012495fe779fda2e&dn=Latex.zip&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2fopen.tracker.cl%3a1337%2fannounce
```

**Web**
```text
Pixeldrain: https://pixeldrain.com/u/9kNKHCSz
```

### Belle Dephine [32cbf6c8]
Dreambooth NSFW
Mot-clé `Belle girl_cute_face`

**Torrent**
```text
magnet:?xt=urn:btih:29a87b4cfa2c3868b54fe382103ad17881b5f868&dn=Belle%20girl_cute_face%20v2%205k.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2fvibe.sleepyinternetfun.xyz%3a1738%2fannounce&tr=udp%3a%2f%2ftracker2.dler.org%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.zerobytes.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.theoks.net%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.publictracker.xyz%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.novaopcj.eu.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.monitorit4.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.encrypted-data.xyz%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.army%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce
```

**Web**
```text
https://drive.google.com/file/d/1hcv-2kznXDYAnUDwUrQm6gHIrUQ0j9bZ/view?usp=share_link
```

### Cal Arts Style
Info & samples https://publicprompts.art/cal-arts-dreambooth-model/
Mot-clé : `CALARTS`

**Web**
```text
https://huggingface.co/PublicPrompts/CALARTS/resolve/main/CALARTS.ckpt
```

### Pixel Landscapes V1
Info & échantillons https://publicprompts.art/pixel-landscapes-v1-dreambooth-model/
Mot-clé: `16-bit-landscape pixel art style`

**Web**
```text
https://huggingface.co/PublicPrompts/16-bit-landscape_PublicPrompts/resolve/main/16-bit-landscape_PublicPrompts.ckpt
```

**Web**
```text
https://drive.google.com/file/d/113V4jfwlgCsAKiMLPgpj0hfdZk1sV5cL/view
```

### MicroWorlds 
Info & échantillons https://publicprompts.art/microworlds-dreambooth-model/
Mot-clé: `microworld render style`

**Web**
```text
https://mega.nz/file/7YJRFbxS#FmTrxB4ayFxUaBz-wiVxHov3YLAUFhObR2JyVwq_p_o
```

**Web**
```text
https://drive.google.com/file/d/1QTUATU3WbYIWX-6ECRypCi1z9TOsXIhG/view?usp=sharing
```

### App Icons Generator V1
Info & échantillons https://publicprompts.art/app-icons-generator-v1-dreambooth-model/
Mot-clé: `SKSKS app icon`

**Web**
```text
https://drive.google.com/file/d/1TNZTQfk0CNZg7nwy033olwOpQAkcglAN/view?usp=sharing
```

### Pixel Art V1
Info & échantillons https://publicprompts.art/pixel-art-v1-dreambooth-model/
Mot-clé: `SKSKS art style`

**Web**
```text
https://drive.google.com/file/d/1HwiqDNm3FyxMNEZLqh7FXsMJv9wmy9bc/view?usp=sharing
```

### VTT RPG 
Une collection de modèles avec les races et les monstres de D&D.
Info & Téléchargements https://huggingface.co/VTTRPGResources

### Comic Diffusion
Dépôt https://huggingface.co/ogkalu/Comic-Diffusion
Comprend 6 styles de Comic.
>The tokens for V2 are:
>`charliebo artstyle`
>`holliemengert artstyle`
>`marioalberti artstyle`
>`pepelarraz artstyle`
>`andreasrocha artstyle`
>`jamesdaly artstyle`

**Web**
```text
https://huggingface.co/ogkalu/Comic-Diffusion/resolve/main/comic-diffusion-V2.ckpt
```

### SD_PixelArt_SpriteSheet_Generator
Permet de générer des feuilles de sprites pixel art sous quatre angles différents..
Dépôt https://huggingface.co/Onodofthenorth/SD_PixelArt_SpriteSheet_Generator
>Pour la vue de face, utilisez `PixelartFSS`
>Pour la vue de droite, utilisez `PixelartRSS`
>Pour la vue arrière, utilisez `PixelartBSS`
>Pour la vue de gauche, utilisez `PixelartLSS`

**Web**
```text
https://huggingface.co/Onodofthenorth/SD_PixelArt_SpriteSheet_Generator/resolve/main/PixelartSpritesheet_V.1.ckpt
```

### midjourney-v4-diffusion
Stable Diffusion affiné sur les images de Midjourney v4 images
Mot-clé : `mdjrny-v4 style`
Dépôt https://huggingface.co/prompthero/midjourney-v4-diffusion

**Web**
```text
https://huggingface.co/prompthero/midjourney-v4-diffusion/resolve/main/mdjrny-v4.ckpt
```

### BloodborneDiffusion
>Il s'agit d'un modèle de Stable Diffusion Dreamboothed entraîné sur la série Bloodborne Style.
>Les données d'entraînement sont composées de 100 images, et l'entraînement a été effectué sur runawayml 1.5 et le nouveau VAE, avec 12k étapes (poly Taux d'apprentissage 1e-6).
>Le mot-clé `Bloodborne Style` apportera le nouveau concept.
>L'échantillonnage recommandé est k_Euler_a ou DPM++ 2M Karras sur 20 étapes, CFG 7 .
Dépôt https://huggingface.co/Guizmus/BloodborneDiffusion

**Web**
```text
https://huggingface.co/Guizmus/BloodborneDiffusion/resolve/main/BloodborneStyle-v1.ckpt
```

### samdoesarts_style [85b77ff9]
>Il s'agit d'un modèle de dreambooth formé sur les travaux de Sam Yang(samdoesarts).
>entraîné avec automatic1111 dreambooth extension
>13000 étapes 128 images d'apprentissage (256 retournées) 1500 images de classe 1e-6 Taux d'apprentissage
>Mot-clé pour le modèle `samdoesarts style`

**Torrent**
```text
magnet:?xt=urn:btih:7999b19ee45d505354e50c25041f405ca8932a38&dn=samdoesarts_style.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://anonfiles.com/JbocEcG4yd/samdoesarts_style_ckpt
```

**Web**
```text
https://pixeldrain.com/u/4s3ntWvR
```

### JWST Deep Space Diffusion
>Il s'agit d'un modèle de Stable Diffusion finement ajusté (basé sur la v1.5) entraîné sur des images prises par le télescope spatial James Webb, ainsi que par Judy Schmidt. Utilisez le mot clé JWST dans vos requêtes pour utiliser le modèle (par exemple, `jwst, galaxie spirale verte`).
Dépôt https://huggingface.co/dallinmackay/JWST-Deep-Space-diffusion

**Web**
```text
https://huggingface.co/dallinmackay/JWST-Deep-Space-diffusion/resolve/main/JWST-Deep-Space.ckpt
```

### copeseethemaldchinai_10000.ckpt (samdoesart) [32186669] 
Encore un autre modèle samdoesart dreambooth.
>100 images
>Pas d'images classe.
>1e-6 Taux d'apprentissage.
>CFG to 9.5
>basé sur "chinai (anything)"
Utilisez `copeseethemald style` dans la requête

**Torrent**
```text
magnet:?xt=urn:btih:4f1365b3dd30541d2c1becfd5d54160928916ba4&dn=copeseethemaldchinai_10000.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://mega.nz/file/xT9jVToK#Sj1S76kl-PC-zCRwJ2FWen6DS0NHY0IXFFAkXhm03eo
```


### CopeSeetheMald-berry200_20400.ckpt (samdoesart) [fa49a214]
Un autre modèle samdoesart dreambooth.
>204 images @ 20.4k étapes
>1e-6 Taux d'apprentissage
>basé sur "berry mix"
Utilisez `copeseethemald style` dans la requête

**Torrent**
```text
magnet:?xt=urn:btih:e4f1f4b9d7d6cf8570914fe22318b44c18d6d602&dn=CopeSeetheMald-berry200_20400.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://mega.nz/folder/1a1xkQQK#4atlB1cJqI35InXxlxyA7A
```



### CopeSeetheMald-200_20400.ckpt (samdoesart) [95f071f9]
Un autre modèle samdoesart dreambooth.
>204 images @ 20.4k étapes
>1e-6 Taux d'apprentissage
>basé sur "blossom mix"
Utilisez `copeseethemald style` dans la requête

**Torrent**
```text
magnet:?xt=urn:btih:9b92aeea5d136ddae25c0d6165405e6e4d144ff1&dn=CopeSeetheMald-200_20400.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://mega.nz/folder/ZG0UnRBJ#jykESWBUCr7hjOoNVTXwLw
```

### Black Souls bs_1500.ckpt [37ec0dc9]
>Un dreambooth formé sur le jeu cg d'un jeu appelé black souls.
>Appellez "Bs" dans la requête.
>avoir vae off, s'assurer que clip skip est réglé sur 1, et régler cfg sur un nombre faible (comme 6).
Aperçu https://cdn.discordapp.com/attachments/1038283286046322819/1040125269761081444/grid-0000.png

**Torrent**
```text
magnet:?xt=urn:btih:63e9c043838f7bca078fb9607873957d11a88e24&dn=bs_1500.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.pomf.se%3a80%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.altrosky.nl%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2ffe.dealclub.de%3a6969%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=udp%3a%2f%2fbt1.archive.org%3a6969%2fannounce&tr=udp%3a%2f%2f6ahddutb1ucc3cp.ru%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce
```

**Web**
```text
https://mega.nz/file/KkIgRY7a#VrKp_JwspTF4e05ApSy4hkJgGUcMKaXSbyB_COZ-o6k
```

### jaggy92500.ckpt [93423a00]
Captures d'écran du jeu.
>Voici les termes à appeler : jaggy, puis ps_game_screenshot, pc_game_screenshot, gamecube_game_screenshot, blood_and_gore, intense_violence, zotov_game_screenshot, eroge_game_screenshot, life_sim_game_screenshot, et/ou alter_echo. Ceci est par ordre décroissant d'influence. Il y en a quelques autres qui ne sont pas assez efficaces pour être nommés, mais qui peuvent avoir des effets forts par moments. Je ne les mentionnerai pas. Vous devriez mettre (cartoon) et peut-être (menu) dans la requête négative.
Apercu: https://imgur.com/a/J4i1DiN

**Torrent**
```text
magnet:?xt=urn:btih:71847b56518e6d7d92dfe24e95f2ee474807b197&dn=jaggy92500.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.cyber-hub.net%3a443%2fannounce
```

**Web**
```text
https://mega.nz/file/0SER2YpC#_MRc6p_sG9cSWqihpt33jpOWyMR8bCZrUaVkh4z5kGE
```

### ykgl.ckpt (y2k cgi girls)
>Il fait des images de filles de l'ère de l'an 2000. Entraînées pour 40 000 étapes.
>Vous les appelez avec `(ykgl cgi_girl)`, ou `(ykgl cgi_girls)`, ou juste `(ykgl girl)`, et ensuite peut-être avec , `cgi_artstyle`.
Aperçu https://imgur.com/a/r2aIcxo

**Web**
```text
https://mega.nz/file/hT0mgTqR#d8g133APl30UtDwsNmzV73_ZESi_kTa5pmQgJoxomn0
```

### CSRmodel (cutesexyrobutts) [b77538cc]
Modèle Dreambooth basé sur le style artistique de cutesexyrobutts.
Aperçu https://i.imgur.com/VPNUae8.png

**Torrent**
```text
magnet:?xt=urn:btih:46c46d2630043f9e47b0107b9680b5bd270263d7&dn=Systemy-CSRmodel.rar&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker1.bt.moack.co.kr%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.dler.org%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.cyber-hub.net%3a443%2fannounce
```

**Web**
```text
https://gofile.io/d/D1L69E
```

### Pepestyle
>Un modèle Dreambooth créé dans le seul but de générer les pepes les plus rares et les plus "dank"..
>StableDiffusion 1.5 a été utilisé comme base pour ce modèle.
>22 images, 400 images de classe, 2.2k étapes à 1.3e-6 Taux d'apprentissage.
>Utilisez la phrase `pepestyle person`
Dépôt https://huggingface.co/SpiteAnon/Pepestyle

**Web**
```text
https://huggingface.co/SpiteAnon/Pepestyle/resolve/main/pepestylev2_2200.ckpt
```

### dbmai [e02601f3]
Un modèle dreambooth de Chine.
Aperçu https://imgur.com/a/fTDgBST
Info https://tieba.baidu.com/p/8136937175

**Torrent**
```text
magnet:?xt=urn:btih:89ede059b4f90887b1f1ce10ba7a37e1d8a0c4b5&dn=dbmai.ckpt&tr=udp%3a%2f%2ftracker.opentrackr.org%3a1337%2fannounce&tr=udp%3a%2f%2fopen.tracker.cl%3a1337%2fannounce&tr=udp%3a%2f%2ftracker.torrent.eu.org%3a451%2fannounce&tr=udp%3a%2f%2fopentracker.i2p.rocks%3a6969%2fannounce&tr=udp%3a%2f%2fopen.stealth.si%3a80%2fannounce&tr=udp%3a%2f%2f9.rarbg.com%3a2810%2fannounce&tr=https%3a%2f%2fopentracker.i2p.rocks%3a443%2fannounce&tr=http%3a%2f%2ftracker.openbittorrent.com%3a80%2fannounce&tr=udp%3a%2f%2ftracker.tiny-vps.com%3a6969%2fannounce&tr=udp%3a%2f%2ftracker.moeking.me%3a6969%2fannounce&tr=udp%3a%2f%2fp4p.arenabg.com%3a1337%2fannounce&tr=udp%3a%2f%2fopen.demonii.com%3a1337%2fannounce&tr=udp%3a%2f%2fmovies.zsw.ca%3a6969%2fannounce&tr=udp%3a%2f%2fipv4.tracker.harry.lu%3a80%2fannounce&tr=udp%3a%2f%2fexplodie.org%3a6969%2fannounce&tr=udp%3a%2f%2fexodus.desync.com%3a6969%2fannounce&tr=https%3a%2f%2ftracker.nanoha.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.lilithraws.org%3a443%2fannounce&tr=https%3a%2f%2ftracker.cyber-hub.net%3a443%2fannounce&tr=https%3a%2f%2ftr.burnabyhighstar.com%3a443%2fannounce
```

**Web**
```text
https://drive.google.com/drive/folders/1MUtM5MTXM35fid5TE3tUSKXAqPP7oysz
```

### gigachad-diffusion
>StableDiffusion v1.5 comme base. 20 images, 400 images de classe, 2.2k étapes à 1e-6 Taux d'apprentissage.
>Mot-clé `gigachad`
Info & Apercu: https://huggingface.co/SpiteAnon/gigachad-diffusion

**Web**
```text
https://huggingface.co/SpiteAnon/gigachad-diffusion/blob/main/gigachad_2000.ckpt
```

### Complex-Lineart
>Entraîné sur environ 100 images à une résolution de 768x768.
>Utiliser la requête: `ComplexLA style`
>Utilisez une résolution proche de 768x768, une résolution inférieure fonctionne mais la qualité ne sera pas aussi bonne.
Dépôt et aperçu des images: https://huggingface.co/Conflictx/Complex-Lineart

**Web**
```text
https://huggingface.co/Conflictx/Complex-Lineart/resolve/main/ComplexLA%20Style.ckpt
```