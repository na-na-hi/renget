...
| DAdaptation     | `--optimizer_args "decouple=True" "weight_decay=0.01"` learning rates between 1.0 and 0.1. Scheduler must be changed to `constant`. Best results with alpha 1? | Adjust learning rate on the fly (adaptive). **Requires arguments to work**. Really high VRAM usage. (8GB Minimum)                                                                             | HIGH! |
...
!!! note More experimentation is needed. It's very time-consuming to test these optimizers, so it's very possible I'm just not using the right parameters, but for the time being, **just use AdamW8bit** unless you have a good reason not to.
I'll update this section if I can find settings that make them comparable or better.

DAdaptation and AdaFactor are *adaptive*, meaning they will automatically adjust learning rates.
~~They need a bunch of different parameters and extra arguments, so I might not have figured out the best settings. But even like that, DAdaptation seemed like the worst of the bunch, most results were unusable or broken, and when coherent, they came out as the image above.~~ Nevermind read below, DAdaptation is a badass.

AdaFactor on the other hand did output results that weren't broken, but the training seemed really weak, which seems to be the common issue with the extra optimizers.
Makes me wonder if those optimizers might be **more suitable for style or concept learning** due to their smaller impact on the output, but I haven't tested it yet.
!!! NOTE Update: Seeing you need very specific parameters to get good results, don't count them out yet.
Since DAdaptation became really good once I found the right usage parameters, I'm looking in AdaFactor's direction for future testing.
While DAdaptation ended up having really good results, it's very heavy and uses 7.1GB of VRAM at batch 1 to operate. AdaFactor is, in turn, rather lightweight, so finding out how to make it work would allow 6GB card users to use an adaptive optimizer, saving a lot of guesswork.
*I'll keep experimenting during the week.*

#### DAdaptation
!!! WARNING DAdaptation needs specific arguments to work. Decoupling the Unet/TE learning rates and a weight_decay value, and set scheduler to `constant`.
!!! WARNING DAdaptation takes command line parameters in order. Make sure to specify Unet learning rate first, or it'll start at 0.5 (or whatever you set for TE LR) learning rate.
![DAdaptation trained model. I somehow gacha'd a SSR medic 2, yay!](https://i.imgur.com/3WrjdJh.png)
Once properly set up, the quality is pretty high, and the text encoder comes out pretty well adjusted in most cases, allowing for flexible outputs.
I take back what I said about `DAdaptation`, the results are actually pretty good as long as it's configured properly.
![I mean it's still pretty nice given the inputs](https://i.imgur.com/67NnDrR.jpg)
This is the worst output I got. It seems to be adding some details that shouldn't be there, and didn't pick with other training methods. Otherwise, it's very good given the inputs.
Big caveat, though. DAdaptation is pretty heavy. **It uses 7.1GB of VRAM at batch size 1, so 6GB VRAM users cannot use it**, unfortunately.
I'll be looking into `AdaFactor` as an alternative.
...
I had a little bit of noise offset (0.06), at those rates it doesn't seem to disturb things at all.
...
This is interesting for a good reason. The more steps you can fit into the training without burning up the Unet or TE, the better. So at times you may want to dampen learning so you can train more. This means more training time, but seems that the results make much better use of the images in addition to their effects.
...
You can still use smaller values for less complex characters or concepts (poses, etc) or a bit higher for styles, but overall don't bother changing it from 128. More net dim might be necessary for models with lots of images or unique tags or trained for a long, long time. But, most of the time, 128 seems to be a fine value.
The only other drawback is file size. Seems to be about `value x 1.3MB`. A hypothetical model with the current safe maximum size, 768, will be roughly 1GB. Double that if saving the model at float precision.
...
⭐So far, 576 (576x576) has been consistently improving my bakes at the cost of training speed.
...
Note that this function **does not work well with adaptive optimizers such as DAdaptation** since learning rates are controlled by the optimizer itself.
