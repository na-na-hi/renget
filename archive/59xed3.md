...
Everything I've trained since I discovered this combination of options has come out pretty good. Some rare cases where it needed 100 steps more or less, but otherwise pretty excellent. With these settings you can be sure your model will come out *alright* as long as the input images aren't terrible.
...
##### 20230503
An 8-bit variant of Lion was recently introduced to the Kohya scripts, but requires an optional update to the bitsandbytes library. I'll be covering it on July. Keep in mind possible damage to your Kohya scripts installation if you proceed, give it a week or two.
#### AdaFactor
...
**20230701**: The quality of this type of training is also boosted by DAdaptAdam training, making it very simple to get a style or character trained without tweaking the settings!
...
!!! WARNING LyCORIS needs to be added to the scripts as a separate package. Use `pip install -U lycoris_lora` with **the virtual env activated** to sort it out.
...
If you were to train a style or concept with a big number of images you may reap some benefits from using LyCORIS and keeping the out file small. For characters it doesn't seem to give any particular advantage other than size and training time, specially since updates to Kohya's scripts now work fine at lower ranks.
