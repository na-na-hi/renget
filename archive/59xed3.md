...
Character LyCORIS uploaded at CivitAI tend to be worse than the equivalent LoRA-LierLa, and *for some unholy reason* have bigger sizes.
You won't get better results than you would with a regular LoRA-LierLa, takes more time/resources, and some of the advantages that sound good on paper don't amount to a lot for generating characters. **Use them for styles and concepts, that's where it shines.** 
Unless your character is very simple and you want very small file sizes, but they'll struggle with anything more complicated than anime waifus with standard clothing and colors.
