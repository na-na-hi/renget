...
### 20230704
Happy burger day! 🍔🦅
I have trained a bunch of models lately and decided to publish my exact settings as "easy mode" training, as it's been working every time, lowering the need to experiment with more complicated techniques with more guesswork. Go to the "easy settings" section for a table.

### 20230702
After some experimenting with LyCORIS I...kind of fail to see the appeal. Doesn't do anything for *model quality*, and doesn't seem to give better results in general terms, compared to a regular LoRA. It does allow for creating smaller files for styles or so, since those benefit from higher rank.
While there's nothing inherently wrong with them, there's also no particular advantage, as results are still heavy dependent on settings, input images and/or tags.
### 20230629
Updated a number of things with newer experiences or adapted to newer updated. Removed a few sections that are now irrelevant.
### 20230623
...
| -------------------------------- | -------- | ------------ | ------ | ------- | -------- | -------------- | ----------- | ---------- |
| Character (good inputs)          | 35-60    | 32-128       | 64-128 | 0.0001  | 0.00005  | No             | 1000+       | 512-768    |
| Character (bad inputs)           | 15-30    | 32-64       | 64-128 | 0.0001  | 0.000045 | Yes            | 1600+       | 512-768    |
| Style                            | 100-4000 | 128-160      | 64-128 | 0.0001  | 0.00004  | No             | ~3000+      | 576-768    |
| Concept                          | 50-2000  | 8(!)-128     | 64-128 | 0.0001  | 0.000045 | TBD            | TBD         | 512-768    |
| ⭐My current settings (characters) | 15-55    | 64          | 64     | 0.0001  | 0.00005  | Either         | ~1800       | 576        |
...
#### The "Easy settings" ⭐⭐
After many tries and experiments I have gotten used to using DAdaptation-Adam as the simplest, easiest way to get a model going.
I've gotten good results with and without tagging, 1000 total steps on average and about 50 images. Works every time and for every character I've thrown at it.
The settings are: 
...
| scheduler         | Constant                                                              |
| Alpha             | 1                                                                     |
| Net dim (Rank)    | 64 or 32                                                              |
| Resolution        | 512 or 576                                                            |
...
Everything I've trained since I discovered this combination of options has come out pretty much good. Some rare cases where it needed 100 steps more or less, but otherwise pretty excellent. With these settings you can be sure your model will come out *alright* as long as the input images aren't terrible.
...
**20230701**: There are newer options like AnyLoRA or such that you can use as well. Personal preference is in effect here, but I still recommend NAI.
AOM3, since it's part of many mixes now, is also a somewhat okay target.
...
| DAdaptAdam     | `--optimizer_args "decouple=True" "weight_decay=0.01" "betas=0.9,0.99"` learning rates between 1.0 and 0.1. Scheduler must be changed to `constant`. Best results with alpha 1? | Adjust learning rate on the fly (adaptive). **Requires arguments to work**. Really high VRAM usage. (8GB Minimum)                                                                             | HIGH!    |
...
#### Updates
**20230628**: Definitely the easiest and simplest way to go if you have at least 7GB of VRAM. Everything I throw at this just works unless the images are really bad (blurry, etc).
...
**20230701**: It seems you don't need to explicitly specify a class name, and the activation word is just enough. `2_robot` is enough for it to be recognized, but this requires having no tag files in the folder.


To activate, use the activation word in a prompt. Hardly used words made up of 3 characters (like the classic "sks", but there are more) work best. You can use any word but it's slightly less efficient.
...
**20230701**: It seems the quality of this type of training is also boosted by DAdaptAdam training, making it very simple to get a style or character trained without tweaking the settings!
...
I personally don't like it just by having to use an extra extension, but there's nothing inherently wrong with them otherwise. 
If you were to train a style or concept with a big number of images you may reap some benefits from using LyCORIS and keeping the out file small. For characters it doesn't seem to give any particular advantage, specially since updates to Kohya's scripts now work fine at lower ranks.
...
IN layers seem to be the "big picture" from where the composition and posture of the final image are determined.
OUT layers seem to give information about the color and texture of the final image.
...
**20230628**:Images with these flaws will *confuse the model* and it'll make similar failures. **Avoid them unless you have no other options!**
...
You may also want closeups of certain details. If you **notice the AI is not giving a specific part enough definition, then a full-resolution image of that part might help**. This could be useful for intricate decorations, accessories, unusual eye designs and so on.
...
**20230628**: This is particularly notable if the images have bad colors or are blurry. In that case, remove those images as soon as possible.
