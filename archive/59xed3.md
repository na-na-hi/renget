...
| Prodigy         | `--optimizer_args "decouple=True" "weight_decay=0.01" "d_coef=2" "use_bias_correction=True" "safeguard_warmup=True"`, d_coef might need adjustment for different types of LyCORIS  | Direct upgrade to `DAdaptation`, viable for SDXL and LyCORIS as well as regular LoRA                                                                                                          | HIGH!    |
...
If you were to train a style or concept with a big number of images you may reap some benefits from using LyCORIS and keeping a small file. For characters it doesn't seem to give any particular advantage other than file size and training time, specially since updates to Kohya's scripts now work fine at lower ranks.
#### Pros
* Cram a lot of training data in a tiny file. 
...
* Certain extensions specific to LoRA will not work with LyCORIS (if you post LyCORIS at CivitAI, *this is why people ask for LoRA versions*).

I don't like LyCORIS a lot, but there's nothing inherently wrong with it either. I don't recommend using it (because you forfeit the use of certain useful extensions by choosing LyCORIS) but it's a perfectly valid option otherwise.
