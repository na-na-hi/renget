...
Out of these five, LoCon, LoHa and DyLoRA are the most interesting ones at the time of writing.
...
If you were to train with a **big number of images** you may reap benefits from using LyCORIS and keeping a small file.

Since LyCORIS has more IN blocks, it might have benefits for subjects with *more unusual body plans* such as robots, animals and furries. But you must have a varied enough training set (if it doesn't work in a LoRA it won't magically work in a LyCORIS, is what I'm saying. If it *already works* in a LoRA, it'll work _better_ in a LyCORIS, instead.). On the other hand, having more blocks will **override style and silhouettes more than LoRA**, so it can complicate training a bit if you **don't** want the style or poses of your training images to be carried over, unless you intentionally use block weighting at training time to skip or lower some blocks.

That said, it's great for styles and concepts and smaller elements, and you can take advantage of the smaller file sizes! 
#### Pros
...
LoRAs have 16 blocks you can train separately, which are "sandwiched" with the checkpoint's Unet in memory.
...
!!! INFO LyCORIS has exactly the same blocks as a checkpoint. 
